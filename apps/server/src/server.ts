...
import { Config, type RuntimeConfig } from "@tinyclaude/config";
import {
	CACHE,
	DEFAULT_STRATEGY,
	getVersion,
...
import { AlertService, APIRouter, AuthService } from "@tinyclaude/http-api";
import {
	LeastUsedStrategy,
	SessionAffinityStrategy,
	SessionStrategy,
} from "@tinyclaude/load-balancer";
import { Logger } from "@tinyclaude/logger";
import { handleResponsesRequest } from "@tinyclaude/openai-responses-adapter";
import {
...
import {
	...
	parseBedrockConfig,
	translateModelName,
} from "@tinyclaude/providers/bedrock";
import {
...
} from "@tinyclaude/proxy";
import { validatePathOrThrow } from "@tinyclaude/security";
import {
	type Account,
...
try {
	const embedded = await import("@tinyclaude/dashboard-web/dist/embedded");
	...
	try {
		const manifestModule = await import(
			"@tinyclaude/dashboard-web/dist/manifest.json"
		);
...
function resolveDashboardAsset(assetPath: string): string | null {
	try {
		...
		return Bun.resolveSync(
			`@tinyclaude/dashboard-web/dist${assetPath}`,
...
async function runStartupMaintenance(
	...
	try {
		...
		const { agentRegistry } = await import("@tinyclaude/agents");
...
export default async function startServer(options?: {
	...
	const apiRouter = new APIRouter({
		...
		getCompressionCacheStats: () =>
			modelCatalogProxyContext?.compressionCache.getStats() ?? null,
	});
	...
	const proxyContext: ProxyContext = {
		...
		asyncWriter,
		compressionCache: new CompressionCache(),
...
`);
	...
	// default, configurable via TINYCLAUDE_MODELS_REFRESH_HOURS; a
