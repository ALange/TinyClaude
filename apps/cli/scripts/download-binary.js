...
function readPackageVersion(scriptDir) {
...
/**
 * Downloads the tinyclaude binary for the current platform/arch and writes
 * it into `<scriptDir>/../dist`. Shared by `postinstall.js` (eager download
 * at install time) and `bin/tinyclaude.js` (lazy download on first run, in
 * case postinstall didn't run — e.g. `bun install -g` blocks lifecycle
 * scripts for untrusted packages by default).
 *
 * @param {{ scriptDir: string }} options - Directory to resolve
 *   `../package.json` and `../dist` relative to. Callers pass their own
 *   `import.meta.url`-derived directory.
 * @returns {Promise<string>} The final path of the downloaded binary.
 * @throws {UnsupportedPlatformError|NotFoundError|Error} On failure.
 */
export async function downloadBinary({ scriptDir }) {
	const version = readPackageVersion(scriptDir);
