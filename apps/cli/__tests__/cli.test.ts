...
describe("CLI Integration Tests", () => {
	...
	beforeEach(() => {
		...
		tempDir = join(tmpdir(), `tinyclaude-test-${Date.now()}`);
	...
	describe("Version Command", () => {
		it("should display version with --version flag", async () => {
			...
			expect(result.stdout).toContain("tinyclaude v");
	...
	describe("Help Command", () => {
		it("should display help with --help flag", async () => {
			...
			expect(result.exitCode).toBe(0);
			expect(result.stdout).toContain("tinyclaude");
	...
	describe("Add Account Command", () => {
		...
		it("should show example usage for add account", async () => {
			...
			expect(output).toMatch(/tinyclaude.*--add-account.*--mode.*--priority/);
		});
	...
	describe("Compress Context Command", () => {
		// IMPORTANT: --compress-context persists to the config file, so every
		// invocation here must point TINYCLAUDE_CONFIG_PATH at a throwaway
		// file inside tempDir to avoid mutating the real user config.
		function configEnv() {
			return {
				TINYCLAUDE_CONFIG_PATH: join(tempDir, "test-config.json"),
			};
		}

		it("should set compress context to true", async () => {
			const result = await runCLI(["--compress-context", "true"], configEnv());

			expect(result.exitCode).toBe(0);
			const output = result.stdout + result.stderr;
			expect(output).toContain("Compress context set to: true");
		});

		it("should set compress context to false", async () => {
			const result = await runCLI(["--compress-context", "false"], configEnv());

			expect(result.exitCode).toBe(0);
			const output = result.stdout + result.stderr;
			expect(output).toContain("Compress context set to: false");
		});

		it("should reject an invalid compress-context value", async () => {
			const result = await runCLI(["--compress-context", "yes"], configEnv());

			expect(result.exitCode).toBe(1);
			const output = result.stdout + result.stderr;
			expect(output).toContain("Invalid value for --compress-context");
		});

		it("should reject --compress-context with a missing value", async () => {
			const result = await runCLI(["--compress-context"], configEnv());

			expect(result.exitCode).toBe(1);
			const output = result.stdout + result.stderr;
			expect(output).toContain("--compress-context requires a value");
		});

		it("should show --compress-context in help", async () => {
			const result = await runCLI(["--help"]);

			expect(result.stdout).toContain("--compress-context <true|false>");
		});
	});
