...
import {
	...
	runDoctor,
	setAccountPriority,
} from "@tinyclaude/cli-commands";
import { Config } from "@tinyclaude/config";
import {
	CLAUDE_MODEL_IDS,
	getVersionSync,
	levenshteinDistance,
	NETWORK,
	shutdown,
} from "@tinyclaude/core";
import { container, SERVICE_KEYS } from "@tinyclaude/core-di";
import { DatabaseFactory } from "@tinyclaude/database";
import { Logger } from "@tinyclaude/logger";
import { parseBedrockConfig } from "@tinyclaude/providers";
// Import server
import startServer from "@tinyclaude/server";
...
function displayConfigInfo(parsed: ParsedArgs, config: Config): void {
	const runtime = config.getRuntime();
	const { getPlatformConfigDir } = require("@tinyclaude/config");
	const { resolveDbPath } = require("@tinyclaude/database");
	...
	configItems.push({
		name: "Host",
		value: process.env.TINYCLAUDE_HOST || "0.0.0.0",
		source: process.env.TINYCLAUDE_HOST
			? "Environment (TINYCLAUDE_HOST)"
			: "Default",
	...
	configItems.push({
		name: "Database Path",
		value: process.env.TINYCLAUDE_DB_PATH || resolveDbPath(),
		source: process.env.TINYCLAUDE_DB_PATH
			? "Environment (TINYCLAUDE_DB_PATH)"
			: "Default",
	...
	configItems.push({
		...
		value: config.getCompressContext(),
		source: process.env.COMPRESS_CONTEXT
			? "Environment (COMPRESS_CONTEXT)"
			: config.get("compress_context") !== undefined
				? "Config file"
				: "Default",
		description: "Compress large tool_result payloads to reduce token usage",
	});
	...
	console.log(`
⚙️  tinyclaude v${version} - Configuration
	...
	for (let i = 0; i < args.length; i++) {
		...
		switch (arg) {
			...
			case "--compress-context":
				if (i + 1 >= args.length || args[i + 1].startsWith("--")) {
					console.error(
						"❌ --compress-context requires a value (true or false)",
					);
					fastExit(1);
				}
				{
					...
					if (value !== "true" && value !== "false") {
						console.error(
							`❌ Invalid value for --compress-context: ${value} (must be "true" or "false")`,
						);
						fastExit(1);
					}
					parsed.compressContext = value === "true";
				}
				break;
...
async function main() {
	...
	if (parsed.version) {
		...
		const version = getVersionSync();
		console.log(`tinyclaude v${version}`);
		fastExit(0);
		return;
	}
	...
	if (parsed.help) {
		...
		const version = getVersionSync();
		console.log(`
🎯 tinyclaude v${version} - Load Balancer for Claude

Usage: tinyclaude [options]
...
Options:
  ...
  --compress-context <true|false>  Enable/disable context compression
...
Examples:
  tinyclaude --serve                # Start server
  tinyclaude --serve --ssl-key /path/to/key.pem --ssl-cert /path/to/cert.pem  # Start server with HTTPS
  tinyclaude --add-account work --mode claude-oauth --priority 0  # Add account
  tinyclaude --add-account my-bedrock --mode bedrock --profile default  # Add Bedrock account
  tinyclaude --reauthenticate work  # Re-authenticate account (preserves metadata)
  tinyclaude --force-reset-rate-limit work  # Force-clear stale rate-limit lock
  tinyclaude --pause work           # Pause account
  tinyclaude --analyze              # Run performance analysis
  tinyclaude --stats                # View stats
  tinyclaude --generate-api-key "My App"  # Generate API-only key
  tinyclaude --generate-api-key "Admin Key" --admin  # Generate admin key
  tinyclaude --list-api-keys               # List all API keys
  tinyclaude --disable-api-key "My App"    # Disable an API key
`);
		fastExit(0);
		return;
	}
	...
	// Handle --compress-context (config-only command, no DB/DI needed)
	if (parsed.compressContext !== null) {
		const config = new Config();
		config.setCompressContext(parsed.compressContext);
		console.log(`✅ Compress context set to: ${parsed.compressContext}`);
		fastExit(0);
		return;
	}
	...
	if (parsed.addAccount) {
		...
		if (!parsed.mode) {
			...
			console.error("\nExample:");
			console.error(
				"  tinyclaude --add-account work --mode claude-oauth --priority 0",
			);
		...
		try {
			...
			if (needsInteractiveAuth) {
				...
				const { stdPromptAdapter } = await import("@tinyclaude/cli-commands");
				...
				let apiKey =
					process.env[
						`TINYCLAUDE_API_KEY_${parsed.addAccount.toUpperCase()}`
					] ||
	...
	console.log("Starting tinyclaude server...");
	const config = new Config();
	startServerWithConfig(parsed, config);
