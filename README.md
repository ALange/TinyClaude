...
tinyclaude supports several environment variables for configuration:

```bash
# Server Configuration
PORT=8080                              # Server port (default: 8080)
TINYCLAUDE_HOST=0.0.0.0           # Server binding host (default: 0.0.0.0, use 127.0.0.1 for localhost-only)
CLIENT_ID=your-client-id              # OAuth client ID
TINYCLAUDE_CONFIG_PATH=/path/to/config.json  # Custom config location
TINYCLAUDE_DB_PATH=/path/to/database.db  # Custom database path (default: ~/.config/tinyclaude/tinyclaude.db)
                                       # Use this for development/testing with a separate database

# Logging and Debugging
LOG_LEVEL=INFO                         # Log level (ERROR, WARN, INFO, DEBUG)
LOG_FORMAT=json                        # Log format (json or text)
TINYCLAUDE_DEBUG=0                  # Enable debug mode (1 for enabled)
...
TINYCLAUDE_OVERLOAD_RETRY_ENABLED=true    # Set to "false" to disable (default: true)
TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS=2  # Total attempts incl. original (default: 2)
TINYCLAUDE_OVERLOAD_RETRY_BASE_MS=750     # Backoff base in ms; 0 = no sleep (default: 750)
TINYCLAUDE_OVERLOAD_RETRY_MAX_MS=3000     # Backoff ceiling in ms (default: 3000)
...
# Agent Discovery
TINYCLAUDE_DISCOVER_PLUGIN_AGENTS=false  # Set to true to discover agents distributed by Claude Code plugins
...
**Security Notes**:
- Use `TINYCLAUDE_HOST=127.0.0.1` to bind only to localhost for better security
...
tinyclaude automatically supports `.env` files for easy configuration management. You can create a `.env` file in your project directory:
...
docker run -v $(pwd)/.env:/app/.env:ro -p 8080:8080 ghcr.io/alange/tinyclaude:latest
```
...
curl -O https://raw.githubusercontent.com/ALange/TinyClaude/main/docker-compose.yml
...
docker run -d \
  --name tinyclaude \
  -p 8080:8080 \
  -v tinyclaude-data:/data \
  -e SSL_KEY_PATH=/path/to/ssl/key.pem \
  -e SSL_CERT_PATH=/path/to/ssl/cert.pem \
  ghcr.io/alange/tinyclaude:latest

# View logs
docker logs -f tinyclaude
```
...
For running tinyclaude as a native systemd service on Linux (without Docker), see the [Systemd Deployment Guide](docs/systemd.md). It covers unit file configuration, memory management with `--smol`, restart policies, and a preflight script that prevents `BUN_JSC_*` environment variable crashes.
...
# Make sure to configure your accounts in the tinyclaude dashboard
...
# If tinyclaude has NO API keys configured (open access):
export ANTHROPIC_AUTH_TOKEN=dummy-key

# If tinyclaude HAS API keys configured (protected):
# Generate a key first: tinyclaude --generate-api-key "My VPS"
export ANTHROPIC_AUTH_TOKEN=btr-abcdef1234567890...  # Use your real tinyclaude API key
...
Use tinyclaude on a trusted server to avoid storing OAuth credentials on untrusted/temporary machines:

**On your trusted server (running tinyclaude):**
```bash
...
tinyclaude --generate-api-key "Remote VPS"
...
```bash
# Set the remote tinyclaude URL and API key
export ANTHROPIC_BASE_URL=https://your-server.com:8080
export ANTHROPIC_AUTH_TOKEN=btr-abcdef1234567890...  # Your tinyclaude API key

# Start Claude CLI (no need to login - tinyclaude handles auth)
claude
```

**How it works:**
- Claude Code CLI sends requests with your tinyclaude API key
- tinyclaude validates the API key and proxies requests using its stored OAuth credentials
...
- **Using only API keys in tinyclaude?** Use Option 2 (logout + API key)
- **Getting auth conflict warnings?** You have both methods active - choose one and follow its steps above

### Codex CLI as a Client

tinyclaude supports [Codex CLI](https://github.com/openai/codex) as a client. Codex speaks the OpenAI Responses API; tinyclaude intercepts requests to `/v1/responses` and `/v1/responses/compact` and translates them to Anthropic `POST /v1/messages` internally, routing through your configured account pool.

Configure Codex CLI to point at tinyclaude in `~/.codex/config.toml`:
...
Codex CLI requires an API key to start — use `dummy-key` if tinyclaude API key authentication is not enabled, or your real tinyclaude API key if it is.
...
To enable HTTPS with tinyclaude, you'll need SSL certificates. Here are your options:

#### Option 1: Generate Self-Signed Certificates (Development/Local Use)

```bash
# Generate a self-signed certificate on the tinyclaude host
openssl req -x509 -newkey rsa:2048 -keyout key.pem -out cert.pem -days 365 -nodes \
  -subj "/C=US/ST=State/L=City/O=Organization/CN=yourhostname"

# Start tinyclaude with SSL
export SSL_KEY_PATH=/path/to/key.pem
export SSL_CERT_PATH=/path/to/cert.pem
tinyclaude

# Or use command line flags
tinyclaude --ssl-key /path/to/key.pem --ssl-cert /path/to/cert.pem
```
...
- **Linux (Ubuntu/Debian):**
  ```bash
  # Copy cert.pem from the tinyclaude host to your client machine
  sudo cp cert.pem /usr/local/share/ca-certificates/tinyclaude.crt
...
- **Linux (Arch/Manjaro):**
  ```bash
  # Copy cert.pem from the tinyclaude host to your client machine
  sudo cp cert.pem /etc/ca-certificates/trust-source/anchors/tinyclaude.crt
...
If you're running tinyclaude on a server with a domain name, use Let's Encrypt or your certificate provider:
...
services:
  ...
  tinyclaude:
    image: ghcr.io/alange/tinyclaude:latest
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.tinyclaude.rule=Host(`your-domain.com`)"
      - "traefik.http.routers.tinyclaude.entrypoints=websecure"
      - "traefik.http.routers.tinyclaude.tls.certresolver=myresolver"
      - "traefik.http.services.tinyclaude.loadbalancer.server.port=8080"
    volumes:
      - ~/.config/tinyclaude:/root/.config/tinyclaude
    restart: unless-stopped
```
...
3. Verify the certificate is in the system store:
   ```bash
   # Linux
   ls -la /etc/ssl/certs/ | grep tinyclaude
...
If you installed tinyclaude via npm on Windows and encounter an error like:

```
The command "C:\Program Files\nodejs\\node_modules\tinyclaude\dist\tinyclaude" is either
...
This bypasses the npm wrapper script entirely and runs tinyclaude directly.

**Option 2: Use the Pre-compiled Binary**

Download the standalone Windows executable from [GitHub Releases](https://github.com/ALange/TinyClaude/releases/latest):

```powershell
# Download tinyclaude-windows-x64.exe and run it directly
.\tinyclaude-windows-x64.exe
```
...
```powershell
node "%APPDATA%\npm\node_modules\tinyclaude\dist\tinyclaude"
```
...
- [@lunetics](https://github.com/lunetics) - Force-reset rate limit feature allowing manual clearing of stale rate-limit locks via API, CLI, and dashboard with immediate usage polling (PR #68), OOM kill prevention with periodic data retention cleanup, 3-day default retention, and time-scoped stats queries (PR #70), model registry sync removing retired models and adding sonnet-4.6 CLI shortcut (PR #71); migrated Anthropic usage handling to the new `limits[]` payload shape — per-account display (session/weekly/per-model rows with severity and overage-credit meter), account ranking/utilization and capacity-guard reads, and model-aware proactive throttling so a per-model weekly cap (e.g. Fable) only throttles matching-model requests instead of the whole account, with flat-window fallback throughout ([#296](https://github.com/ALange/TinyClaude/pull/296)); surfaced usage-window exhaustion that was invisible to both `/health` and the accounts dashboard during the 2026-07-09 incident — added a `usage_exhausted` pool counter (subset of `routable`), extracted `rateLimitStatus` display precedence into a tested pure helper (usage exhaustion > unified header snapshot > legacy cooldown > OK), and a shared staleness guard so a known-past window reset is never reported as exhausted on either surface, with characterization tests pinning the incident's account-wide-bench-to-pool-flap chain ([#299](https://github.com/ALange/TinyClaude/pull/299)); fixed the agent interceptor silently rewriting subagent request models to stale/retired ids via a hardcoded alias map, and unconditionally overriding explicit per-call model choices — rewrites now fire only on an explicit DB preference, with a live Anthropic model catalog (`GET /v1/models`, weekly refresh, console-API accounts only by default to avoid OAuth ban risk, passive capture from pass-through traffic) vetting rewrite targets and backing dashboard model dropdowns/validation; added `original_model`/`applied_model` request-row provenance and an `x-tinyclaude-model-rewrite` response header, post-rewrite-aware account/combo routing, agent model provenance (`modelSource`, revert-to-inherit) with a redesigned two-mechanism dashboard edit dialog separating the agent-file default from the runtime preference override ([#300](https://github.com/ALange/TinyClaude/pull/300))
