...
import type { RuntimeConfig } from "@tinyclaude/config";
import type { Disposable } from "@tinyclaude/core";
...
export class DatabaseOperations implements StrategyStore, Disposable {
	...
	constructor(
		...
		if (isPostgres) {
			...
			const pgMax = Number(process.env.TINYCLAUDE_DB_POOL_MAX ?? 10);
			const pgIdleTimeout = Number(process.env.TINYCLAUDE_DB_IDLE_TIMEOUT ?? 0);
			...
			const requestedPgStatementTimeout = Number(
				process.env.TINYCLAUDE_DB_STATEMENT_TIMEOUT,
			);
			...
			const pgPrepare = process.env.TINYCLAUDE_DB_PG_PREPARE === "true";
	...
	generateRecoveryInstructions(): string {
		const dbPath = this.resolvedDbPath ?? "~/.config/tinyclaude/tinyclaude.db";
		return `
...
`.trim();
	...
	async saveCompressionEvents(
		...
		await withDatabaseRetry(
			() =>
				this.requests.saveCompressionEvents(
