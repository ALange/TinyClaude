...
function makeTempDbPath(): string {
	return path.join(
		fs.mkdtempSync(path.join(os.tmpdir(), "tinyclaude-autovac-test-")),
		"test.db",
	);
}
...
describe("DatabaseOperations.bootstrapAutoVacuum", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "tinyclaude-bootstrap-test-"),
		);
...
describe("DatabaseOperations.incrementalVacuum: no destructive fallback", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "tinyclaude-incvac-test-"));
		dbPath = path.join(tmpDir, "test.db");
	});
