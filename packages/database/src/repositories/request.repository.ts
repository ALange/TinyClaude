	...
	async save(data: RequestData): Promise<void> {
		...
		await this.run(
			...
			INSERT INTO requests (
				...
				alignment_score, volatile_findings_count
			)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			ON CONFLICT (id) DO UPDATE SET
				...
				alignment_score = COALESCE(EXCLUDED.alignment_score, requests.alignment_score),
				volatile_findings_count = COALESCE(EXCLUDED.volatile_findings_count, requests.volatile_findings_count)
		`,
			[
				data.id,
				Date.now(),
				...
				data.volatileFindingsCount ?? null,
			],
		);
	}
	...
	async saveCompressionEvents(
		...
		await this.run(
			`INSERT INTO compression_events (
				id, request_id, content_type, compressor_used, strategy,
				compression_ratio, tokens_before, tokens_after, chars_before, chars_after,
				cache_hit, timestamp
			) VALUES ${rows}`,
			params,
		);
