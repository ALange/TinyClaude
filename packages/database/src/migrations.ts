 ...
 *
 * Override with `TINYCLAUDE_MIGRATION_BACKUP_KEEP`. 0 disables pruning
...
function getBackupRetention(): number {
	const raw = process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP;
	if (raw === undefined || raw === "") return DEFAULT_BACKUP_RETENTION;
	if (!INTEGER_RE.test(raw)) {
		log.warn(
			`TINYCLAUDE_MIGRATION_BACKUP_KEEP="${raw}" is not a non-negative integer — using default ${DEFAULT_BACKUP_RETENTION}`,
		);
...
export function ensureSchema(db: Database): void {
	...
	db.run(`
		CREATE TABLE IF NOT EXISTS requests (
			...
			volatile_findings_count INTEGER
		)
	`);
	...
	// (may not exist if table was inherited from an older database)
	const payloadCols = (
	...
	// Create compression_events table: per-tool_result-block compression
	// outcomes recorded by the proxy's compression step (content_type is
	// nullable — cache-hit rows have no fresh content-type detection).
	db.run(`
		CREATE TABLE IF NOT EXISTS compression_events (
			id TEXT PRIMARY KEY,
			request_id TEXT NOT NULL,
			content_type TEXT,
			compressor_used TEXT NOT NULL,
			strategy TEXT,
	...
	`);
	db.run(
		`CREATE INDEX IF NOT EXISTS idx_compression_events_request_id ON compression_events(request_id)`,
	);
	db.run(
		`CREATE INDEX IF NOT EXISTS idx_compression_events_timestamp ON compression_events(timestamp DESC)`,
	);
}
...
export function runMigrations(db: Database, dbPath?: string): void {
	...
	const migrationTx = db.transaction(() => {
		...
		// Add alignment_score / volatile_findings_count columns if they don't exist
		// (cache-alignment scoring for the compression insights feature)
		if (!requestsColumnNames.includes("alignment_score")) {
			db.prepare(
				"ALTER TABLE requests ADD COLUMN alignment_score INTEGER",
			).run();
			log.info("Added alignment_score column to requests table");
		}
		if (!requestsColumnNames.includes("volatile_findings_count")) {
			db.prepare(
				"ALTER TABLE requests ADD COLUMN volatile_findings_count INTEGER",
			).run();
			log.info("Added volatile_findings_count column to requests table");
		}
