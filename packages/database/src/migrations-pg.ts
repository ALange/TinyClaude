...
export async function ensureSchemaPg(adapter: BunSqlAdapter): Promise<void> {
	...
	// Create compression_events table (see SQLite migration for rationale).
	await adapter.unsafe(`
...
export async function runMigrationsPg(adapter: BunSqlAdapter): Promise<void> {
	...
	const columnsToAdd: Array<{
		...
		},
		{
			...
			definition: "ALTER TABLE requests ADD COLUMN alignment_score INTEGER",
		},
		{
			table: "requests",
			column: "volatile_findings_count",
			definition:
	...
	// Ensure compression_events table exists (for upgrades from pre-compression-insights installs)
	await adapter.unsafe(`
		CREATE TABLE IF NOT EXISTS compression_events (
			id TEXT PRIMARY KEY,
			request_id TEXT NOT NULL,
