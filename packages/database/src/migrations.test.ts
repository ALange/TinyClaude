...
describe("Database Migrations - Tier Column Removal", () => {
	...
	describe("Compression & Cache-Alignment Insights (alignment_score/volatile_findings_count, compression_events)", () => {
		it("ensureSchema creates alignment_score and volatile_findings_count INTEGER columns on requests", () => {
			ensureSchema(db);
			...
			const alignmentScore = columns.find((c) => c.name === "alignment_score");
			const volatileFindingsCount = columns.find(
				(c) => c.name === "volatile_findings_count",
			);

			expect(alignmentScore).toBeDefined();
			expect(alignmentScore?.type.toUpperCase()).toBe("INTEGER");
			expect(volatileFindingsCount).toBeDefined();
			expect(volatileFindingsCount?.type.toUpperCase()).toBe("INTEGER");
		});

		it("ensureSchema creates the compression_events table with its indexes", () => {
			ensureSchema(db);

			const tables = db
				.prepare(
					"SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'compression_events'",
				)
				.all() as Array<{ name: string }>;
			expect(tables).toHaveLength(1);

			const columns = db
				.prepare("PRAGMA table_info(compression_events)")
				.all() as Array<{ name: string }>;
			const columnNames = columns.map((c) => c.name);
			...
			const indexes = db
				.prepare(
					"SELECT name FROM sqlite_master WHERE type = 'index' AND tbl_name = 'compression_events'",
				)
				.all() as Array<{ name: string }>;
			const indexNames = indexes.map((idx) => idx.name);
			expect(indexNames).toContain("idx_compression_events_request_id");
			expect(indexNames).toContain("idx_compression_events_timestamp");
		});

		it("runMigrations adds alignment_score/volatile_findings_count columns idempotently to a pre-existing requests table", () => {
			// Simulate a legacy DB without the new columns by building the requests
			// table manually (mirrors ensureSchema's pre-migration shape) instead of
			// calling ensureSchema, which already includes the columns under test.
			db.run(`
			...
			const columnNames = columns.map((c) => c.name);
			expect(columnNames).toContain("alignment_score");
			expect(columnNames).toContain("volatile_findings_count");
...
describe("Database Backup Behavior", () => {
	...
	it("should prune older backups to TINYCLAUDE_MIGRATION_BACKUP_KEEP", () => {
		...
		const prev = process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP;
		process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP = "2";
		try {
			...
			if (prev === undefined) {
				delete process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP;
			} else {
				process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP = prev;
			}
		}
	...
	it("should fall back to the default retention for garbled env values", () => {
		...
		const prev = process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP;
		process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP = "2abc";
		try {
			const db2 = new Database(dbPath);
			db2.prepare("ALTER TABLE accounts ADD COLUMN account_tier TEXT").run();
			db2.close();
	...
	it("should leave backups in place when TINYCLAUDE_MIGRATION_BACKUP_KEEP=0", () => {
		...
		const prev = process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP;
		process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP = "0";
		try {
			const db2 = new Database(dbPath);
			db2.prepare("ALTER TABLE accounts ADD COLUMN account_tier TEXT").run();
			db2.close();
	...
	it("should fall back to the default retention for negative env values", () => {
		...
		const prev = process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP;
		process.env.TINYCLAUDE_MIGRATION_BACKUP_KEEP = "-1";
		try {
			const db2 = new Database(dbPath);
			db2.prepare("ALTER TABLE accounts ADD COLUMN account_tier TEXT").run();
			db2.close();
