...
describe("getModelCatalogOAuthRefreshEnabled", () => {
	const originalEnv = process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH;

	beforeEach(() => {
		delete process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH;
	});
	...
	afterEach(() => {
		if (originalEnv === undefined) {
			...
			process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH = originalEnv;
		}
	});
	...
	it("honors a truthy env override", () => {
		process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH = "1";
		const { config, cleanup } = makeConfig();
		try {
	...
	it("treats a non-true env value as disabled", () => {
		process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH = "disabled";
		const { config, cleanup } = makeConfig();
		try {
			expect(config.getModelCatalogOAuthRefreshEnabled()).toBe(false);
		} finally {
	...
	it("prioritizes the env override over a config-file value", () => {
		process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH = "false";
		const { config, cleanup } = makeConfig();
		try {
			config.set("model_catalog_oauth_refresh_enabled", true);
			expect(config.getModelCatalogOAuthRefreshEnabled()).toBe(false);
