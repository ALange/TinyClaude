...
export class Config extends EventEmitter {
	...
	getCompressContext(): boolean {
		const fromEnv = process.env.COMPRESS_CONTEXT;
		if (fromEnv) {
			return fromEnv !== "false" && fromEnv !== "0";
		}
		const fromFile = this.data.compress_context;
		if (typeof fromFile === "boolean") return fromFile;
		return false; // default: disabled
	}

	setCompressContext(value: boolean): void {
		this.set("compress_context", value);
	}
	...
	getModelCatalogOAuthRefreshEnabled(): boolean {
		const fromEnv = parseEnabledEnvFlag(
			process.env.TINYCLAUDE_MODELS_OAUTH_REFRESH,
		);
		if (fromEnv !== undefined) {
			return fromEnv;
		}
	...
	getAllSettings(): Record<string, string | number | boolean | undefined> {
		...
		return {
			...
			compress_context: this.getCompressContext(),
...
// Re-export types
export type { StrategyName } from "@tinyclaude/core";
