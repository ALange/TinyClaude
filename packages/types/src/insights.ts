...
/**
 * Response types for the compression insights endpoint (/api/insights/compression).
 */

export interface CompressionInsightsMeta {
	range: string;
}

export interface CompressionInsightsTotals {
	requests: number;
...
export interface CompressionInsightsRow {
	key: string;
	events: number;
...
export interface CompressionInsightsResponse {
	meta: CompressionInsightsMeta;
	totals: CompressionInsightsTotals;
	byContentType: CompressionInsightsRow[];
	byCompressor: CompressionInsightsRow[];
	liveCache: {
		entries: number;
		stableHashes: number;
		hits: number;
...
/** GET/POST response and request shapes for /api/config/compression. */
export interface CompressionGetResponse {
	compressContext: boolean;
}

export interface CompressionSetRequest {
	compressContext: boolean;
}
