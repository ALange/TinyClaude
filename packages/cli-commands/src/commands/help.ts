...
export function getHelpText(): string {
	return `
Usage: tinyclaude <command> [options]
...
Examples:
  tinyclaude add myaccount --mode claude-oauth --priority 10
  tinyclaude add grok --mode xai --priority 50
  tinyclaude add anthropic-account --mode anthropic-compatible --priority 5 --modelMappings '{"opus":"claude-3-opus","sonnet":"claude-3-sonnet"}'
  tinyclaude add "My Account" --mode claude-oauth --priority 10  # Account names with spaces must be quoted
  tinyclaude list
  tinyclaude remove myaccount
  tinyclaude pause "My Account"  # Use quotes for names with spaces
  tinyclaude resume myaccount
  tinyclaude set-priority myaccount 20
  tinyclaude token-health
  tinyclaude reauth-needed
