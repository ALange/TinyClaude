 ...
 * tinyclaude server. Running compact concurrently with a live server
 ...
 * `systemctl is-active tinyclaude` (should report `inactive` or
		...
		if (reason !== null) {
			throw new Error(
				`Refusing to compact: ${reason}. ` +
					`Stop the tinyclaude service first (e.g. \`systemctl stop tinyclaude\` ` +
					`then \`systemctl is-active tinyclaude\` to confirm it's not auto-restarting) ` +
