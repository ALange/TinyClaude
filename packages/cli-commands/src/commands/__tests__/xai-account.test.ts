...
describe("CLI xAI account import", () => {
	...
	beforeEach(() => {
		...
		previousGrokAuthPath = process.env.TINYCLAUDE_GROK_AUTH_PATH;
		process.env.TINYCLAUDE_GROK_AUTH_PATH = grokAuthPath;
	...
	afterEach(() => {
		...
		if (previousGrokAuthPath === undefined) {
			delete process.env.TINYCLAUDE_GROK_AUTH_PATH;
		} else {
			process.env.TINYCLAUDE_GROK_AUTH_PATH = previousGrokAuthPath;
		}
