...
import { shutdown } from "@tinyclaude/core";
		...
		switch (command) {
			case "add": {
				...
				if (!name) {
					...
					console.log(
						"Usage: tinyclaude-cli add <name> [--mode <claude-oauth|console|codex|qwen|xai|zai|minimax|anthropic-compatible|openai-compatible|nanogpt|kilo|openrouter|ollama|ollama-cloud>] [--priority <number>] [--modelMappings <JSON>]",
					);
					process.exit(1);
				}
			...
			case "remove": {
				...
				if (!name) {
					console.error("Error: Account name is required");
					console.log("Usage: tinyclaude-cli remove <name> [--force]");
					process.exit(1);
				}
			...
			case "pause": {
				...
				if (!name) {
					console.error("Error: Account name is required");
					console.log("Usage: tinyclaude-cli pause <name>");
					process.exit(1);
				}
			...
			case "resume": {
				...
				if (!name) {
					console.error("Error: Account name is required");
					console.log("Usage: tinyclaude-cli resume <name>");
					process.exit(1);
				}
			...
			case "set-priority": {
				...
				if (!name) {
					console.error("Error: Account name is required");
					console.log("Usage: tinyclaude-cli set-priority <name> <priority>");
					process.exit(1);
				}
