...
describe("AgentRegistry plugin agent discovery", () => {
	let tmpDir: string;
	const originalEnv = process.env.TINYCLAUDE_DISCOVER_PLUGIN_AGENTS;

	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "bcf-agents-test-"));
		delete process.env.TINYCLAUDE_DISCOVER_PLUGIN_AGENTS;
	});

	afterEach(() => {
		fs.rmSync(tmpDir, { recursive: true, force: true });
		if (originalEnv !== undefined) {
			process.env.TINYCLAUDE_DISCOVER_PLUGIN_AGENTS = originalEnv;
		} else {
			delete process.env.TINYCLAUDE_DISCOVER_PLUGIN_AGENTS;
		}
	});
	...
	it("loads plugin agents when env var is set to 'true' and manifest is valid", async () => {
		process.env.TINYCLAUDE_DISCOVER_PLUGIN_AGENTS = "true";
