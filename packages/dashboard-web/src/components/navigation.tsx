import { parseHttpError } from "@tinyclaude/errors";
import {
	Activity,
	BarChart3,
	Bot,
...
let updateCommand: string = "npm install -g tinyclaude@latest";
...
export function Navigation({
	...
	const navItems: NavItem[] = useMemo(() => {
		const baseItems: NavItem[] = [
			...
			{ label: "Compression", icon: Minimize2, path: "/compression" },
			{ label: "Requests", icon: Activity, path: "/requests" },
			{ label: "Accounts", icon: Users, path: "/accounts" },
	...
	const getUpdateCommand = (
		...
		if (isDocker) {
			...
			return "docker pull ghcr.io/alange/tinyclaude:latest";
		}
		...
		switch (packageManager) {
			case "bun":
				return "bun install -g tinyclaude@latest";
			default:
				return "npm install -g tinyclaude@latest";
		}
	};
