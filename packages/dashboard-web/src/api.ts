...
class API extends HttpClient {
	private static readonly API_KEY_STORAGE_KEY = "tinyclaude-api-key";
	...
	async getCompressionInsights(
		range = "24h",
	): Promise<CompressionInsightsResponse> {
		const params = new URLSearchParams({ range });

		return this.get<CompressionInsightsResponse>(
			`/api/insights/compression?${params.toString()}`,
		);
	}
	...
	async getCompressionSettings(): Promise<CompressionGetResponse> {
		const startTime = Date.now();
		const url = "/api/config/compression";

		this.logger.debug(`→ GET ${url}`);

		try {
			const response = await this.get<CompressionGetResponse>(url);
			const duration = Date.now() - startTime;
			this.logger.debug(`← GET ${url} - 200 (${duration}ms)`);
			return response;
		} catch (error) {
	...
	async setCompressionSettings(compressContext: boolean): Promise<void> {
		const startTime = Date.now();
		const url = "/api/config/compression";
		const partial: CompressionSetRequest = { compressContext };
	...
	async getAlerts(limit = 100): Promise<{
		alerts: import("@tinyclaude/types").AlertEvent[];
