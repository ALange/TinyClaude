...
import { sanitizeProxyHeaders } from "@tinyclaude/http-common";
import { Logger } from "@tinyclaude/logger";
import { resolveReasoningEffort } from "@tinyclaude/openai-formats";
...
const CODEX_SYNTHETIC_COUNT_TOKENS_URL =
	"https://tinyclaude.local/codex/count_tokens";
...
export class CodexProvider extends BaseProvider {
	name = "codex";
	// Fallback map: proxy-operations.ts injects x-tinyclaude-request-id and
	// x-tinyclaude-request-stream into the upstream response before calling
	...
	async transformRequestBody(
		...
		try {
			...
			const requestId = request.headers.get("x-tinyclaude-request-id");
			if (requestId) {
	...
	async processResponse(
		...
		const requestId = response.headers.get("x-tinyclaude-request-id");
	...
	private createSyntheticJsonResponse(
		...
		const headers = new Headers({
			...
			"x-tinyclaude-synthetic-response": "true",
			"x-tinyclaude-synthetic-status": String(status),
		});
		return new Request(request.url, {
			method: request.method,
			headers,
	...
	private async transformSseResponseToJson(
		...
		const requestId =
			response.headers.get("x-tinyclaude-request-id") ?? "unknown";
