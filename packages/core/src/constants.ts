/**
 * Centralized constants for the tinyclaude application
...
export const TIME_CONSTANTS = {
	...
	// Override at runtime via TINYCLAUDE_DEFAULT_COOLDOWN_NO_RESET_MS.
	...
	// Override at runtime via TINYCLAUDE_RATE_LIMIT_BACKOFF_BASE_MS /
	// TINYCLAUDE_RATE_LIMIT_BACKOFF_MAX_MS / TINYCLAUDE_RATE_LIMIT_RESET_STABILITY_MS.
 ...
 * Reads BASE/MAX from env (TINYCLAUDE_RATE_LIMIT_BACKOFF_BASE_MS /
 * TINYCLAUDE_RATE_LIMIT_BACKOFF_MAX_MS), falling back to TIME_CONSTANTS.
...
export function computeRateLimitBackoffMs(consecutiveCount: number): number {
	...
	const baseEnv = Number(process.env.TINYCLAUDE_RATE_LIMIT_BACKOFF_BASE_MS);
	const maxEnv = Number(process.env.TINYCLAUDE_RATE_LIMIT_BACKOFF_MAX_MS);
 ...
 * Reads TINYCLAUDE_RATE_LIMIT_RESET_STABILITY_MS from env.
 * Uses || (not ??) so 0/NaN env values fall through to the default.
 */
export function getRateLimitResetStabilityMs(): number {
	const raw = Number(process.env.TINYCLAUDE_RATE_LIMIT_RESET_STABILITY_MS);
 ...
 *
 * Env knobs:
 *   TINYCLAUDE_OVERLOAD_RETRY_ENABLED      — set to "false" to disable (default: true)
 *   TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS — max in-place attempts (default: 2)
 *   TINYCLAUDE_OVERLOAD_RETRY_BASE_MS      — jitter backoff base delay ms (default: 750)
 *   TINYCLAUDE_OVERLOAD_RETRY_MAX_MS       — jitter backoff ceiling ms (default: 3000)
 */
export function getOverloadRetryConfig(): {
	...
	const enabled = process.env.TINYCLAUDE_OVERLOAD_RETRY_ENABLED !== "false";
	// maxAttempts: 0 is not useful (use ENABLED=false to disable), so || is correct.
	const maxAttempts =
		Number(process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS) || 2;
	// baseMs/maxMs: 0 is valid (zero delay for tests), so use explicit finite check.
	const rawBase = Number(process.env.TINYCLAUDE_OVERLOAD_RETRY_BASE_MS);
	const rawMax = Number(process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_MS);
