...
// Build-time injected version via --define __TINYCLAUDE_VERSION__="x.y.z"
// Replaced by bun bundler with a string literal; undefined at dev/runtime.
declare const __TINYCLAUDE_VERSION__: string | undefined;
...
export async function getVersion(): Promise<string> {
	...
	if (typeof __TINYCLAUDE_VERSION__ !== "undefined" && __TINYCLAUDE_VERSION__) {
		cachedVersion = __TINYCLAUDE_VERSION__;
		return cachedVersion;
	}

	// 2. Runtime env var fallback (dev/test environments)
	if (process.env.TINYCLAUDE_VERSION) {
		cachedVersion = process.env.TINYCLAUDE_VERSION;
		return cachedVersion;
	}
