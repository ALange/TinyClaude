import type { Config } from "@tinyclaude/config";
import {
	DEFAULT_AGENT_MODEL,
	NETWORK,
	STRATEGIES,
...
export function createConfigHandlers(
	...
	return {
		...
		/**
		 * Get current compress-context setting
		 */
		getCompression: (): Response => {
			return jsonResponse({
				compressContext: config.getCompressContext(),
			});
		},

		/**
		 * Set compress-context setting
		 */
		setCompression: async (req: Request): Promise<Response> => {
			const body = (await req.json()) as CompressionSetRequest;
			if (typeof body.compressContext !== "boolean") {
				return errorResponse(
					BadRequest("Invalid 'compressContext': must be boolean"),
				);
			}
			config.setCompressContext(body.compressContext);
			return new Response(null, { status: 204 });
		},
