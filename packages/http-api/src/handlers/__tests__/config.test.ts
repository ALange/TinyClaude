...
import type { APIContext } from "@tinyclaude/types";
...
function makeConfig() {
	return {
		...
		setCompressContext: mock(() => {}),
	} as unknown as import("@tinyclaude/config").Config;
}
...
describe("createConfigHandlers", () => {
	...
	it("returns the current compressContext setting", async () => {
		const handlers = createConfigHandlers(makeConfig(), {
			port: 8080,
			tlsEnabled: false,
		});

		const response = handlers.getCompression();
		const body = (await response.json()) as Record<string, unknown>;

		expect(body.compressContext).toBe(true);
	});

	it("updates compressContext from a valid POST body", async () => {
		const config = makeConfig();
		const handlers = createConfigHandlers(config, {
			port: 8080,
			tlsEnabled: false,
		...
		const response = await handlers.setCompression(
			new Request("http://localhost/api/config/compression", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ compressContext: false }),
			}),
		);

		expect(response.status).toBe(204);
		expect(config.setCompressContext).toHaveBeenCalledWith(false);
	});

	it("rejects a non-boolean compressContext with 400", async () => {
		const config = makeConfig();
		const handlers = createConfigHandlers(config, {
			port: 8080,
			tlsEnabled: false,
		...
		const response = await handlers.setCompression(
			new Request("http://localhost/api/config/compression", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ compressContext: "yes" }),
			}),
		);

		expect(response.status).toBe(400);
		expect(config.setCompressContext).not.toHaveBeenCalled();
	});
