import { getModelRates, type ModelRates } from "@tinyclaude/core";
import {
	errorResponse,
	InternalServerError,
	jsonResponse,
} from "@tinyclaude/http-common";
...
} from "../services/compression-insights";
import {
...
/** Raw GROUP BY row shape for compression_events, grouped by content_type or compressor_used. */
interface CompressionEventGroupSqlRow {
	key: string | null;
	events: number;
...
function toCompressionEventGroupRow(
	row: CompressionEventGroupSqlRow,
): CompressionEventGroupRow {
	return {
		key: row.key,
		events: Number(row.events) || 0,
		cacheHitEvents: Number(row.cache_hit_events) || 0,
		tokensSaved: Number(row.tokens_saved) || 0,
		sumRatio: Number(row.sum_ratio) || 0,
		ratioCount: Number(row.ratio_count) || 0,
	};
}
...
/**
 * GET /api/insights/compression — compression & cache-alignment insights.
 *
 * Aggregates `compression_events` rows (joined to `requests` for range/filter
 * scoping) grouped by content_type and by compressor_used, plus alignment
 * scoring stats from `requests` directly, plus the live in-memory
 * CompressionCache stats. See services/compression-insights.ts's module docs
 * for why cache-hit rows are excluded from the tokens-saved/ratio sums.
 */
export function createCompressionInsightsHandler(context: APIContext) {
	return async (searchParams: URLSearchParams): Promise<Response> => {
		const db = context.dbOps.getAdapter();
		const { startMs, range } = getRangeConfig(
			searchParams.get("range") ?? "24h",
		...
		try {
			const byContentTypeSqlRows = await db.query<CompressionEventGroupSqlRow>(
				`
				SELECT
					ce.content_type as key,
					COUNT(*) as events,
					SUM(CASE WHEN ce.cache_hit = 1 THEN 1 ELSE 0 END) as cache_hit_events,
					SUM(CASE WHEN ce.cache_hit = 0 THEN COALESCE(ce.tokens_before, 0) - COALESCE(ce.tokens_after, 0) ELSE 0 END) as tokens_saved,
					SUM(CASE WHEN ce.cache_hit = 0 AND ce.compression_ratio IS NOT NULL THEN ce.compression_ratio ELSE 0 END) as sum_ratio,
					SUM(CASE WHEN ce.cache_hit = 0 AND ce.compression_ratio IS NOT NULL THEN 1 ELSE 0 END) as ratio_count
			...
			const byCompressorSqlRows = await db.query<CompressionEventGroupSqlRow>(
				`
				SELECT
					ce.compressor_used as key,
					COUNT(*) as events,
					SUM(CASE WHEN ce.cache_hit = 1 THEN 1 ELSE 0 END) as cache_hit_events,
					SUM(CASE WHEN ce.cache_hit = 0 THEN COALESCE(ce.tokens_before, 0) - COALESCE(ce.tokens_after, 0) ELSE 0 END) as tokens_saved,
					SUM(CASE WHEN ce.cache_hit = 0 AND ce.compression_ratio IS NOT NULL THEN ce.compression_ratio ELSE 0 END) as sum_ratio,
				...
				JOIN requests r ON r.id = ce.request_id
				WHERE ${whereClause}
				GROUP BY ce.compressor_used
			`,
				queryParams,
			);

			const byContentType = byContentTypeSqlRows.map(
				toCompressionEventGroupRow,
			);
			const byCompressor = byCompressorSqlRows.map(toCompressionEventGroupRow);

			// Top-level totals derived from byCompressor's raw SQL rows (not
			// byContentType) — compressor_used is NOT NULL on every event
			// (real compressor name or "cache_hit"), so summing it covers
			// every row exactly once. Avoids a third DB round trip.
			let compressionEventsCount = 0;
			let cacheHitEventsCount = 0;
			let totalTokensSavedFreshOnly = 0;
			let sumRatioFreshOnly = 0;
			let ratioCountFreshOnly = 0;
			for (const row of byCompressor) {
				compressionEventsCount += row.events;
				cacheHitEventsCount += row.cacheHitEvents;
				totalTokensSavedFreshOnly += row.tokensSaved;
				sumRatioFreshOnly += row.sumRatio;
				ratioCountFreshOnly += row.ratioCount;
			}

			const [alignmentSqlRow] = await db.query<{
			...
			const requestCount = Number(alignmentSqlRow?.request_count) || 0;
			const avgAlignmentScore =
				alignmentSqlRow?.avg_alignment_score == null
					? null
					: Number(alignmentSqlRow.avg_alignment_score);

			const stats = context.getCompressionCacheStats?.() ?? null;
			const liveCache = stats
				? {
						entries: stats.entries ?? 0,
						stableHashes: stats.stableHashes ?? 0,
						hits: stats.hits ?? 0,
			...
			return jsonResponse(
				buildCompressionInsightsResponse({
					range,
					requestCount,
					compressionEventsCount,
					cacheHitEventsCount,
					totalTokensSavedFreshOnly,
					sumRatioFreshOnly,
					ratioCountFreshOnly,
					avgAlignmentScore,
					byContentType,
			...
			log.error("Compression insights error:", error);
			return errorResponse(
				InternalServerError("Failed to fetch compression insights data"),
			);
		}
	};
}
