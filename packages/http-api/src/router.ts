import { validateNumber } from "@tinyclaude/core";
import { Unauthorized } from "@tinyclaude/errors";
import {
...
import {
	...
	createCompressionInsightsHandler,
...
export class APIRouter {
	...
	private registerHandlers(): void {
		...
		const compressionInsightsHandler = createCompressionInsightsHandler(
			this.context,
		);
		...
		this.handlers.set("GET:/api/config/compression", () =>
			configHandlers.getCompression(),
		);
		this.handlers.set("POST:/api/config/compression", (req) =>
			configHandlers.setCompression(req),
		);
		...
		this.handlers.set("GET:/api/insights/compression", (_req, url) =>
			compressionInsightsHandler(url.searchParams),
		);
		// Alert routes
