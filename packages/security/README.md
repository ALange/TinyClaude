...
Centralized security utilities for path validation and sanitization in the tinyclaude monorepo.
...
```typescript
import { validatePath } from '@tinyclaude/security';
...
- TinyClaude config directory (`~/.config/tinyclaude` on Linux/macOS, `AppData/Local/tinyclaude` on Windows)
...
```typescript
import { getValidationCacheSize, clearValidationCache } from '@tinyclaude/security';
...
Part of the tinyclaude project.
