...
export function getDefaultAllowedBasePaths(forceRefresh = false): string[] {
	...
	// TinyClaude config directory (OS-independent)
	try {
		const home = homedir();
		if (home) {
			...
			if (platform === "win32") {
				...
				configDir = join(baseDir, "tinyclaude");
			} else {
 ...
 * The default allowed paths (tinyclaude config directory, current directory, temp directory)
