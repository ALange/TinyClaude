...
function getCacheDir(): string {
	return (
		process.env.TINYCLAUDE_MODELS_CACHE_DIR || join(tmpdir(), "tinyclaude")
	);
}
...
function getRefreshHours(): number {
	const raw = process.env.TINYCLAUDE_MODELS_REFRESH_HOURS;
...
function isOffline(): boolean {
	return process.env.TINYCLAUDE_MODELS_OFFLINE === "1";
}
	...
	if (!account) {
		throw new Error(
			allowOAuth
				...
				: "No active anthropic account available to fetch models (console/API-key accounts only; set TINYCLAUDE_MODELS_OAUTH_REFRESH=1 or use a manual refresh to allow an OAuth account fallback)",
		);
	}
...
export async function refreshModelCatalog(
	...
	if (isOffline()) {
		...
		return {
			success: false,
			error: "Model catalog refresh is disabled (TINYCLAUDE_MODELS_OFFLINE=1)",
			catalog,
		};
	}
 ...
 * `nextRefreshAt` down if `TINYCLAUDE_MODELS_REFRESH_HOURS` was lowered
 ...
 * Set `TINYCLAUDE_MODELS_REFRESH_HOURS=0` to disable entirely.
