...
import { registerHeartbeat } from "@tinyclaude/core";
...
export class CacheKeepaliveScheduler {
	...
	private async replayRequest(accountId: string): Promise<void> {
		...
		try {
			...
			replayHeaders.set("x-tinyclaude-account-id", accountId);
			replayHeaders.set("x-tinyclaude-bypass-session", "true");
			...
			replayHeaders.set("x-tinyclaude-keepalive", "true");
