...
describe("getModelList — model_fallbacks merge", () => {
	it("merges model_fallbacks into the model list", async () => {
		const { getModelList } = await import("@tinyclaude/core");
		const account = makeAccount({
			model_mappings: JSON.stringify({ sonnet: "qwen/qwen3.6-plus:free" }),
			model_fallbacks: JSON.stringify({
				sonnet: "bytedance-seed/dola-seed-2.0-pro:free",
...
describe("proxyWithAccount — 529 in-place retry", () => {
	...
	beforeEach(() => {
		...
		process.env.TINYCLAUDE_OVERLOAD_RETRY_BASE_MS = "0";
		process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_MS = "0";
		delete process.env.TINYCLAUDE_OVERLOAD_RETRY_ENABLED;
		delete process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS;
	});

	afterEach(() => {
		globalThis.fetch = originalFetch;
		delete process.env.TINYCLAUDE_OVERLOAD_RETRY_BASE_MS;
		delete process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_MS;
		delete process.env.TINYCLAUDE_OVERLOAD_RETRY_ENABLED;
		delete process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS;
	});
	...
	it("retries in-place on 529 no-reset and makes exactly 2 fetch calls before succeeding", async () => {
		...
		process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS = "2";
	...
	it("falls through to cooldown/failover when all retries are exhausted", async () => {
		...
		process.env.TINYCLAUDE_OVERLOAD_RETRY_MAX_ATTEMPTS = "3";
		const ctx = make529NoResetCtx();
		const bodyBuffer = makeRequestBody();
		const result = await proxyWithAccount(
			makeRequest(bodyBuffer),
	...
	it("skips in-place retry when TINYCLAUDE_OVERLOAD_RETRY_ENABLED=false", async () => {
		let callCount = 0;
		globalThis.fetch = mock(async () => {
			callCount++;
			return new Response(overloadBody, {
		...
		process.env.TINYCLAUDE_OVERLOAD_RETRY_ENABLED = "false";
		const ctx = make529NoResetCtx();
		const bodyBuffer = makeRequestBody();
		await proxyWithAccount(
			makeRequest(bodyBuffer),
