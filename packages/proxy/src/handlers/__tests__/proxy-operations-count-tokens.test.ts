...
import { CodexProvider } from "@tinyclaude/providers";
...
describe("proxyWithAccount — Codex count_tokens", () => {
	...
	it("does not trust client-supplied synthetic response markers", async () => {
		...
		try {
			...
			const result = await proxyWithAccount(
				makeMessagesRequest(bodyBuffer, {
					...
					"x-tinyclaude-synthetic-status": "418",
				}),
		...
		expect(
			fetchedRequest?.headers.get("x-tinyclaude-synthetic-response"),
		).toBeNull();
		expect(
			fetchedRequest?.headers.get("x-tinyclaude-synthetic-status"),
		).toBeNull();
	});
});
