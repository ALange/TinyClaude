...
import type { Account, ComboWithSlots, RequestMeta } from "@tinyclaude/types";
import {
	getComboSlotInfo,
...
describe("selectAccountsForRequest — x-tinyclaude-account-id header", () => {
	it("returns exactly the forced account when the header matches", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({ "x-tinyclaude-account-id": "acc-2" }),
		});
	...
	it("falls through to normal selection when forced account id is not found", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({ "x-tinyclaude-account-id": "nonexistent" }),
		});
	...
	it("falls through to normal selection when forced account is paused", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({ "x-tinyclaude-account-id": "acc-paused" }),
		});
	...
	it("falls through to normal selection when forced account is rate-limited", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({ "x-tinyclaude-account-id": "acc-rl" }),
		});
...
describe("selectAccountsForRequest — auto-refresh bypass (overage-paused accounts)", () => {
	 ...
	 * due to auto_pause_on_overage. It sends x-tinyclaude-bypass-session: true
	 * alongside x-tinyclaude-account-id. The selector must allow these through
	...
	it("allows overage-paused account when bypass-session header is present", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({
				"x-tinyclaude-account-id": "acc-overage",
				"x-tinyclaude-bypass-session": "true",
			}),
		});
	...
	it("blocks manually-paused overage-enabled account even with bypass-session header", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({
				"x-tinyclaude-account-id": "acc-manual",
				"x-tinyclaude-bypass-session": "true",
			}),
		});
	...
	it("allows rate-limited (non-paused) account when bypass-session header is present", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({
				"x-tinyclaude-account-id": "acc-rl",
				"x-tinyclaude-bypass-session": "true",
			}),
		});
	...
	it("blocks failure-paused account even with bypass-session header", async () => {
		...
		const meta = makeRequestMeta({
			headers: new Headers({
				"x-tinyclaude-account-id": "acc-broken",
				"x-tinyclaude-bypass-session": "true",
			}),
		});
