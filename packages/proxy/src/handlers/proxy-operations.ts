...
import { withSanitizedProxyHeaders } from "@tinyclaude/http-common";
import { Logger } from "@tinyclaude/logger";
import { stripCacheControlFromOpenAIRequest } from "@tinyclaude/openai-formats";
import {
	getProvider,
	isAnthropicOutOfCredits,
	usageCache,
} from "@tinyclaude/providers";
import type { Account, RateLimitReason, RequestMeta } from "@tinyclaude/types";
...
const SYNTHETIC_RESPONSE_HEADER = "x-tinyclaude-synthetic-response";
const SYNTHETIC_STATUS_HEADER = "x-tinyclaude-synthetic-status";
const SYNTHETIC_RESPONSE_URL_PREFIX = "https://tinyclaude.local/";
 ...
 *      60s by default, overridable via TINYCLAUDE_DEFAULT_COOLDOWN_NO_RESET_MS) as
...
export function extractCooldownUntil(
	...
	const DEFAULT_COOLDOWN_MS =
		Number(process.env.TINYCLAUDE_DEFAULT_COOLDOWN_NO_RESET_MS) ||
		...
		return forwardToClient(
			{
				...
				alignmentScore: requestMeta.alignmentScore,
				volatileFindingsCount: requestMeta.volatileFindingsCount,
...
export async function proxyWithAccount(
	...
	try {
		...
		const isSyntheticInternal =
			!!req.headers.get("x-tinyclaude-keepalive") ||
			!!req.headers.get("x-tinyclaude-auto-refresh");
		...
		responseHeaders.set("x-tinyclaude-request-id", requestMeta.id);
		const internalRequestStream = transformedRequest.headers.get(
			"x-tinyclaude-request-stream",
		);
		if (internalRequestStream === "true" || internalRequestStream === "false") {
			responseHeaders.set("x-tinyclaude-request-stream", internalRequestStream);
		}
			...
			if (rlInfo.isRateLimited && !rlInfo.resetTime) {
				...
				if (retryCfg.enabled && retryCfg.maxAttempts > 1) {
					for (let attempt = 1; attempt < retryCfg.maxAttempts; attempt++) {
						...
						retryTaggedHeaders.set("x-tinyclaude-request-id", requestMeta.id);
	...
	return new Response(
		...
		}),
		{
			...
			headers: {
				...
				"x-tinyclaude-pool-status": "exhausted",
			},
		},
	);
}
