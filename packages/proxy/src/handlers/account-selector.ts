import { getModelFamily, isAccountAvailable } from "@tinyclaude/core";
...
export async function selectAccountsForRequest(
	...
	if (meta.headers) {
		const forcedAccountId = meta.headers.get("x-tinyclaude-account-id");
		if (forcedAccountId) {
			try {
				const allAccounts = await ctx.dbOps.getAllAccounts();
				...
				if (forcedAccount) {
					// The auto-refresh scheduler sends dummy messages with x-tinyclaude-bypass-session
					...
					const isAutoRefreshBypass =
						meta.headers.get("x-tinyclaude-bypass-session") === "true";
	...
	const excludeProviders =
		meta.headers
			?.get("x-tinyclaude-exclude-providers")
			?.split(",")
			.map((p) => p.trim())
			.filter(Boolean) ?? [];
