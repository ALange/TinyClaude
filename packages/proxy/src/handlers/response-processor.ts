import { getRateLimitResetStabilityMs, logError } from "@tinyclaude/core";
...
export async function processProxyResponse(
	...
	if (rateLimitInfo.isRateLimited) {
		...
		const isKeepalive =
			requestMeta?.headers?.get("x-tinyclaude-keepalive") === "true";
		if (isKeepalive) {
			log.warn(
		...
		const bypassSession =
			requestMeta?.headers?.get("x-tinyclaude-bypass-session") === "true";
