...
import type { BunSqlAdapter } from "@tinyclaude/database";
import { Logger } from "@tinyclaude/logger";
import { fetchUsageData, getProvider } from "@tinyclaude/providers";
		...
		try {
			...
			const accounts = await this.db.query<{
				...
				WHERE
					...
					-- tinyclaude already knows upstream will reject us until rate_limited_until,
	...
	private async sendDummyMessage(accountRow: {
		...
		try {
			...
			const headers = new Headers({
				...
				"x-tinyclaude-account-id": account.id,
				...
				// bug 2). Mirrors the existing x-tinyclaude-keepalive
