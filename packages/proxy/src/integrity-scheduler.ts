import { TIME_CONSTANTS } from "@tinyclaude/core";
import type { DatabaseOperations } from "@tinyclaude/database";
import { runIntegrityCheckInWorker } from "@tinyclaude/database";
import { Logger } from "@tinyclaude/logger";

/**
 * Periodic integrity scheduler. Two probes run on independent timers:
 *
 *  - **quick** (`PRAGMA quick_check`) every `TINYCLAUDE_INTEGRITY_CHECK_INTERVAL`
 *    hours (default 6). Catches page-structure corruption and most
 *    freelist issues.
 *  - **full** (`PRAGMA integrity_check` + `PRAGMA foreign_key_check`) every
 *    `TINYCLAUDE_FULL_INTEGRITY_CHECK_INTERVAL` hours (default 24). Catches
 ...
 * `TINYCLAUDE_FULL_INTEGRITY_MAX_DB_BYTES` (0 = no limit / never skip).
 */
 ...
 * `TINYCLAUDE_FULL_INTEGRITY_MAX_DB_BYTES`: unset/empty → default; positive
...
function resolveMaxFullIntegrityBytes(logger: Logger): number {
	const raw = process.env.TINYCLAUDE_FULL_INTEGRITY_MAX_DB_BYTES;
	if (raw === undefined || raw === "")
		return DEFAULT_FULL_INTEGRITY_MAX_DB_BYTES;
	const parsed = Number(raw);
	if (!Number.isInteger(parsed) || parsed < 0) {
		logger.warn(
			`Invalid TINYCLAUDE_FULL_INTEGRITY_MAX_DB_BYTES="${raw}", using default ${DEFAULT_FULL_INTEGRITY_MAX_DB_BYTES}`,
		);
		return DEFAULT_FULL_INTEGRITY_MAX_DB_BYTES;
	}
...
export function startIntegrityScheduler(
	...
	if (quickInterval !== null) {
		...
		logger.info(
			"Quick integrity check disabled (TINYCLAUDE_INTEGRITY_CHECK_INTERVAL=0)",
		);
	}
	...
	if (fullInterval !== null) {
		...
		logger.info(
			"Full integrity check disabled (TINYCLAUDE_FULL_INTEGRITY_CHECK_INTERVAL=0)",
		);
	}
 ...
 *    `TINYCLAUDE_FULL_INTEGRITY_MAX_DB_BYTES`, the full `integrity_check` can't
		...
		if (kind === "full") {
			...
			if (sizeBytes > maxBytes) {
				const reason = `full integrity_check skipped — DB is ${(
					...
					1,
				)} GB threshold, TINYCLAUDE_FULL_INTEGRITY_MAX_DB_BYTES); ran quick_check instead`;
				logger.warn(reason);
