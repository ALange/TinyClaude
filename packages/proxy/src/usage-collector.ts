...
import { AsyncDbWriter, DatabaseOperations } from "@tinyclaude/database";
import { Logger } from "@tinyclaude/logger";
import { NO_ACCOUNT_ID, type RequestResponse } from "@tinyclaude/types";
		...
		this.asyncWriter.enqueue(async () => {
			try {
				await this.dbOps.saveRequest(
					...
					startMessage.alignmentScore ?? undefined,
					startMessage.volatileFindingsCount ?? undefined,
				);
			} catch (error) {
				log.error(
