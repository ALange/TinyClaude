...
//
// The guard is: isSyntheticInternal = !!req.headers.get("x-tinyclaude-auto-refresh")
...
describe("proxy-operations — isSyntheticInternal header detection", () => {
	it("header x-tinyclaude-auto-refresh: true is truthy (probe detected)", () => {
		const req = new Request("https://proxy.local/v1/messages", {
			method: "POST",
			headers: { "x-tinyclaude-auto-refresh": "true" },
		});
	...
	it("header x-tinyclaude-auto-refresh absent is falsy (normal request)", () => {
		const req = new Request("https://proxy.local/v1/messages", {
			method: "POST",
	...
	it("keepalive header also triggers isSyntheticInternal (existing guard preserved)", () => {
		const req = new Request("https://proxy.local/v1/messages", {
			method: "POST",
			headers: { "x-tinyclaude-keepalive": "1" },
		});
		const isSyntheticInternal =
			!!req.headers.get("x-tinyclaude-keepalive") ||
			!!req.headers.get("x-tinyclaude-auto-refresh");
...
describe("proxy.ts — pool-exhausted path skips usageCollector for auto-refresh probes", () => {
	...
	beforeEach(() => {
		savedPassthrough = process.env.TINYCLAUDE_PASSTHROUGH_ON_EMPTY_POOL;
		delete process.env.TINYCLAUDE_PASSTHROUGH_ON_EMPTY_POOL;
	});
	...
	afterEach(() => {
		if (savedPassthrough === undefined) {
			...
			process.env.TINYCLAUDE_PASSTHROUGH_ON_EMPTY_POOL = savedPassthrough;
		}
	});
