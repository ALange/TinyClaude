...
// @tinyclaude/database. The scheduler imports `runIntegrityCheckInWorker`
...
mock.module("@tinyclaude/database", () => ({
...
describe("startIntegrityScheduler", () => {
	afterEach(() => {
		delete process.env.TINYCLAUDE_INTEGRITY_CHECK_INTERVAL;
		delete process.env.TINYCLAUDE_FULL_INTEGRITY_CHECK_INTERVAL;
	});
	...
	it("TINYCLAUDE_INTEGRITY_CHECK_INTERVAL=0 disables only the quick check", () => {
		process.env.TINYCLAUDE_INTEGRITY_CHECK_INTERVAL = "0";
		const dbOps = makeDbOps();
	...
	it("setting both env vars to 0 returns a no-op stop", () => {
		process.env.TINYCLAUDE_INTEGRITY_CHECK_INTERVAL = "0";
		process.env.TINYCLAUDE_FULL_INTEGRITY_CHECK_INTERVAL = "0";
		const dbOps = makeDbOps();
		const stop = startIntegrityScheduler(dbOps);
		expect(() => stop()).not.toThrow();
		expect(
	...
	it("garbled env values fall back to default", () => {
		process.env.TINYCLAUDE_INTEGRITY_CHECK_INTERVAL = "6abc";
		const dbOps = makeDbOps();
		const stop = startIntegrityScheduler(dbOps, { fullIntervalHours: 500 });
		expect(typeof stop).toBe("function");
		stop();
