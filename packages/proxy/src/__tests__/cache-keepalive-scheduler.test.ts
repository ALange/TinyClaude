 ...
 *
 * Strategy: mock.module("@tinyclaude/core") so registerHeartbeat is
...
// module resolution picks up the mock when it resolves @tinyclaude/core.
// ---------------------------------------------------------------------------
...
describe("CacheKeepaliveScheduler", () => {
	...
	describe("sendKeepalives() (triggered via heartbeat callback)", () => {
		...
		it("with one cached account — fetch called once with correct URL and headers", async () => {
			...
			expect(headers.get("x-tinyclaude-account-id")).toBe(accountId);
			expect(headers.get("x-tinyclaude-bypass-session")).toBe("true");
