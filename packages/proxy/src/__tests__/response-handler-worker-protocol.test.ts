...
describe("forwardToClient usage-collector protocol", () => {
	...
	it("passes alignmentScore and volatileFindingsCount when provided", async () => {
		const { starts } = createMockCollector();
		const ctx = createCtx();

		await forwardToClient(
			{
				requestId: "req-alignment-metrics",
				method: "POST",
				path: "/v1/messages",
				account: null,
				requestHeaders: new Headers({ "content-type": "application/json" }),
		...
		expect(starts[0].type).toBe("start");
		expect(starts[0].alignmentScore).toBe(85);
		expect(starts[0].volatileFindingsCount).toBe(3);
	});

	it("defaults alignmentScore and volatileFindingsCount to null when omitted", async () => {
		const { starts } = createMockCollector();
		const ctx = createCtx();

		await forwardToClient(
			{
				requestId: "req-no-alignment-metrics",
				method: "POST",
				path: "/v1/messages",
				account: null,
				requestHeaders: new Headers({ "content-type": "application/json" }),
		...
		expect(starts[0].type).toBe("start");
		expect(starts[0].alignmentScore).toBeNull();
		expect(starts[0].volatileFindingsCount).toBeNull();
	});
