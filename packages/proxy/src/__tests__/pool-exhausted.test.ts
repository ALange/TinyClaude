...
describe("pool exhausted — 503 response", () => {
	...
	it("returns x-tinyclaude-pool-status: exhausted header", async () => {
		const ctx = makeContext([]);
		const response = await handleProxy(
			makeRequest(),
			new URL("https://proxy.local/v1/messages"),
		...
		expect(response.status).toBe(503);
		expect(response.headers.get("x-tinyclaude-pool-status")).toBe("exhausted");
	});
...
describe("pool exhausted — TINYCLAUDE_PASSTHROUGH_ON_EMPTY_POOL=1 escape hatch", () => {
	it("does NOT return 503 when TINYCLAUDE_PASSTHROUGH_ON_EMPTY_POOL=1 and pool is empty", async () => {
		process.env.TINYCLAUDE_PASSTHROUGH_ON_EMPTY_POOL = "1";
