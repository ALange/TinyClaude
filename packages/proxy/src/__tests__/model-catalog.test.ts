...
import { getProvider } from "@tinyclaude/providers";
...
const TEST_CACHE_DIR = join(tmpdir(), "tinyclaude-test-model-catalog");
...
describe("model-catalog", () => {
	...
	beforeEach(async () => {
		process.env.TINYCLAUDE_MODELS_CACHE_DIR = TEST_CACHE_DIR;
		delete process.env.TINYCLAUDE_MODELS_REFRESH_HOURS;
		delete process.env.TINYCLAUDE_MODELS_OFFLINE;
		await cleanCacheDir();
		resetModelCatalogForTest();
	});
	...
	afterEach(async () => {
		...
		delete process.env.TINYCLAUDE_MODELS_CACHE_DIR;
		delete process.env.TINYCLAUDE_MODELS_REFRESH_HOURS;
		delete process.env.TINYCLAUDE_MODELS_OFFLINE;
		await cleanCacheDir();
		resetModelCatalogForTest();
	...
	describe("refreshModelCatalog / getModelCatalog", () => {
		...
		it("treats TINYCLAUDE_MODELS_OFFLINE=1 as a no-op refresh", async () => {
			process.env.TINYCLAUDE_MODELS_OFFLINE = "1";
		...
		it("fails an automatic-trigger refresh against an OAuth-only account when the opt-in is not set", async () => {
			...
			expect(result.success).toBe(false);
			expect(result.error).toMatch(/TINYCLAUDE_MODELS_OAUTH_REFRESH/);
		});
		...
		it("persists nextRefreshAt computed from the configured refresh interval", async () => {
			process.env.TINYCLAUDE_MODELS_REFRESH_HOURS = "2";
			global.fetch = mock(
				async () =>
					new Response(JSON.stringify({ data: [], has_more: false }), {
						status: 200,
	...
	describe("initModelCatalogRefresh", () => {
		it("disables the scheduler entirely when refresh hours is 0 (no fetch, inert unregister)", async () => {
			process.env.TINYCLAUDE_MODELS_REFRESH_HOURS = "0";
		...
		it("fires the initial refresh once the freshly-derived due time has already passed", async () => {
			...
			process.env.TINYCLAUDE_MODELS_REFRESH_HOURS = "0.0000000001";
			const fetchMock = mock(
				async () =>
					new Response(
						JSON.stringify({
		...
		it("resumes a refresh from a persisted nextRefreshAt that has already passed", async () => {
			process.env.TINYCLAUDE_MODELS_REFRESH_HOURS = "168";
			await writeCacheFile({
				models: [
					{ id: "old-model", displayName: "Old Model", createdAt: null },
				],
		...
		it("does not refresh before a persisted future nextRefreshAt", async () => {
			process.env.TINYCLAUDE_MODELS_REFRESH_HOURS = "1";
			await writeCacheFile({
				models: [
					{ id: "old-model", displayName: "Old Model", createdAt: null },
				],
	...
	describe("ingestModelsListing", () => {
		...
		it("is a no-op when TINYCLAUDE_MODELS_OFFLINE=1", async () => {
			process.env.TINYCLAUDE_MODELS_OFFLINE = "1";
