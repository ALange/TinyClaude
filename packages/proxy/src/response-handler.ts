import { requestEvents, TIME_CONSTANTS } from "@tinyclaude/core";
import {
	sanitizeRequestHeaders,
	withSanitizedProxyHeaders,
} from "@tinyclaude/http-common";
import { Logger } from "@tinyclaude/logger";
import type { Account, RateLimitReason } from "@tinyclaude/types";
import {
...
const MODEL_REWRITE_HEADER = "x-tinyclaude-model-rewrite";
...
export async function forwardToClient(
	...
	const isAutoRefreshProbe =
		requestHeaders.get("x-tinyclaude-auto-refresh") === "true";
	...
	if (shouldProcessRequest) {
		const startMessage: StartMessage = {
			...
			volatileFindingsCount: volatileFindingsCount ?? null,
		};
