# tinyclaude Configuration Guide

This guide covers all configuration options for tinyclaude, including file-based configuration, environment variables, and runtime API updates.
...
tinyclaude uses a flexible configuration system that supports:
...
Configuration is managed through the `@tinyclaude/config` package, which provides automatic loading, validation, and change notifications.
...
2. **Configuration file** - Values from `~/.config/tinyclaude/tinyclaude.json` (or custom path)
...
- **Linux/macOS**: `~/.config/tinyclaude/tinyclaude.json` (or `$XDG_CONFIG_HOME/tinyclaude/tinyclaude.json`)
- **Windows**: `%LOCALAPPDATA%\tinyclaude\tinyclaude.json` (or `%APPDATA%\tinyclaude\tinyclaude.json`)
- **Custom path**: Set via `TINYCLAUDE_CONFIG_PATH` environment variable
...
| `TINYCLAUDE_DEBUG` | string | - | Set to `"1"` to enable debug mode with console output |
...
| `TINYCLAUDE_CONFIG_PATH` | - | string | `TINYCLAUDE_CONFIG_PATH=/etc/tinyclaude.json` |
...
| `TINYCLAUDE_DEBUG` | Enable debug mode with console output | - | `TINYCLAUDE_DEBUG=1` |
| `TINYCLAUDE_DB_PATH` | Custom database file path (SQLite only) | Platform-specific | `TINYCLAUDE_DB_PATH=/var/lib/tinyclaude/db.sqlite` |
| `DATABASE_URL` | Use PostgreSQL instead of SQLite. Set to a `postgresql://` or `postgres://` connection string. When set, `TINYCLAUDE_DB_PATH` is ignored. | - | `DATABASE_URL=postgresql://user:pass@localhost:5432/tinyclaude` |
...
| `TINYCLAUDE_MODELS_REFRESH_HOURS` | Hours between scheduled model catalog refreshes; `0` disables scheduled refresh entirely | `168` (7 days) | `TINYCLAUDE_MODELS_REFRESH_HOURS=48` |
| `TINYCLAUDE_MODELS_OFFLINE` | Disable scheduled/manual model catalog refresh **and** passive `/v1/models` capture | - | `TINYCLAUDE_MODELS_OFFLINE=1` |
| `TINYCLAUDE_MODELS_CACHE_DIR` | Directory for the persisted model catalog cache file. Use a persistent directory (not a tmpdir that's wiped on restart) to keep the refresh schedule stable across restarts | Platform tmp dir | `TINYCLAUDE_MODELS_CACHE_DIR=/var/lib/tinyclaude` |
| `TINYCLAUDE_MODELS_OAUTH_REFRESH` | Allow OAuth accounts as a fallback source for *scheduled* (automatic) model catalog refreshes when no console/API-key account is eligible. Same as the `model_catalog_oauth_refresh_enabled` config file field; env var takes precedence. Manual refreshes (`POST /api/models/refresh`) always allow the OAuth fallback regardless of this setting | - (console-only) | `TINYCLAUDE_MODELS_OAUTH_REFRESH=1` |
...
tinyclaude supports two database backends:

| Backend | When used | Environment variable |
|---------|-----------|---------------------|
| **SQLite** (default) | `DATABASE_URL` is not set, or starts with `sqlite://` | `TINYCLAUDE_DB_PATH` (optional path override) |
| **PostgreSQL** | `DATABASE_URL` starts with `postgres://` or `postgresql://` | `DATABASE_URL=postgresql://user:pass@host:5432/db` |

### SQLite (default)

No configuration required. The database is created automatically in the platform-specific directory (`~/.config/tinyclaude/tinyclaude.db`).

```bash
# Optional: store database at a custom path
export TINYCLAUDE_DB_PATH=/var/lib/tinyclaude/tinyclaude.db
```
...
```bash
export DATABASE_URL=postgresql://tinyclaude_user:secret@localhost:5432/tinyclaude
```
...
|----------|---------|-------------|
| `TINYCLAUDE_DB_POOL_MAX` | `10` | Maximum pooled connections |
| `TINYCLAUDE_DB_IDLE_TIMEOUT` | `0` (disabled) | Seconds before an idle pooled connection is closed |
| `TINYCLAUDE_DB_STATEMENT_TIMEOUT` | `7000` | Server-side statement timeout in milliseconds (clamped below the 8000ms client-side timeout) |
| `TINYCLAUDE_DB_PG_PREPARE` | `false` | Set to `true` to re-enable named prepared statement caching |
...
- **Scheduled (automatic) refresh**: only console/API-key accounts are eligible, unless `TINYCLAUDE_MODELS_OAUTH_REFRESH=1` (or the `model_catalog_oauth_refresh_enabled` config field) opts in to an OAuth fallback.
...
When at least one eligible account exists, the catalog refreshes automatically on a schedule controlled by `TINYCLAUDE_MODELS_REFRESH_HOURS` (default **168 hours / 7 days**; `0` disables scheduled refresh). To avoid many independently-restarting instances all hitting Anthropic at the same wall-clock moment, each scheduled refresh is smeared with random jitter (up to 24 hours) on top of the configured interval. Every successful catalog write — scheduled, manual, or passive (see below) — recomputes and persists the next scheduled refresh time, so the schedule stays anchored to the most recent real data rather than drifting.

### Passive capture

Any successful `GET /v1/models` response proxied through tinyclaude from a console/API-key account (a client calling the pass-through endpoint directly) is also captured into the catalog cache opportunistically, independent of the scheduled/manual refresh paths. This never triggers extra outbound calls — it only observes traffic that was going to happen anyway.

### Bundled fallback

If no live fetch has ever succeeded (fresh install, no eligible account, or `TINYCLAUDE_MODELS_OFFLINE=1`), `GET /api/models` serves a static list bundled with tinyclaude (`CLAUDE_MODEL_IDS` in `packages/core/src/models.ts`). Its response reports `source: "fallback"` and a `fetchedAt` timestamp equal to the bundled list's snapshot date (`BUNDLED_MODELS_AS_OF`), not the current time — this is an intentional, honest "as of `<date>`" provenance rather than a `Date.now()` that would misleadingly imply the list was just fetched. The dashboard surfaces this distinction next to the model catalog's refresh button ("Live model list · fetched ..." vs. "Bundled model list · as of ...").
...
⚠️ **NOTE**: Only the `"session"` strategy is available in tinyclaude. Other strategies (round-robin, least-requests, weighted) have been removed from the codebase as they can trigger Claude's anti-abuse systems and result in account bans.
...
tinyclaude --add-account primary-account --mode max --priority 0

# Add secondary accounts with lower priorities
tinyclaude --add-account secondary-1 --mode max --priority 10
tinyclaude --add-account secondary-2 --mode max --priority 20

# Add backup account with lowest priority
tinyclaude --add-account backup --mode console --priority 50

# Enable auto-fallback on primary account (API call)
ACCOUNT_ID=$(tinyclaude --list | grep "primary-account" | jq -r '.id')
...
tail -f ~/.local/share/tinyclaude/logs/tinyclaude.log | grep "Auto-fallback"
...
tinyclaude performs validation on:
...
1. Create the configuration file:
   ```bash
   mkdir -p ~/.config/tinyclaude
   ```

2. Export current configuration:
   ```bash
   curl http://localhost:8080/api/config > ~/.config/tinyclaude/tinyclaude.json
   ```

3. Edit and format the file:
   ```bash
   jq '.' ~/.config/tinyclaude/tinyclaude.json > temp.json && mv temp.json ~/.config/tinyclaude/tinyclaude.json
   ```
...
1. **Configuration location**: Move from `~/.tinyclaude/config.json` to platform-specific paths
...
```bash
cp ~/.config/tinyclaude/tinyclaude.json ~/.config/tinyclaude/tinyclaude.json.backup
```
...
1. **Configuration not loading**:
   - Check file permissions: `ls -la ~/.config/tinyclaude/`
   - Verify JSON syntax: `jq '.' ~/.config/tinyclaude/tinyclaude.json`
