# tinyclaude CLI Documentation

The tinyclaude CLI provides a command-line interface for managing OAuth accounts, monitoring usage statistics, and controlling the load balancer.
...
```
🎯 tinyclaude - Load Balancer for Claude
...
tinyclaude stores its configuration in platform-specific directories:

#### macOS/Linux
```
~/.config/tinyclaude/tinyclaude.json
```

Or if `XDG_CONFIG_HOME` is set:
```
$XDG_CONFIG_HOME/tinyclaude/tinyclaude.json
```

#### Windows
```
%LOCALAPPDATA%\tinyclaude\tinyclaude.json
```

Or fallback to:
```
%APPDATA%\tinyclaude\tinyclaude.json
```
...
- **Windows**: `%LOCALAPPDATA%\tinyclaude\tinyclaude.db`
...
|----------|-------------|---------|
| `TINYCLAUDE_CONFIG_PATH` | Override config file location | Platform default |
| `TINYCLAUDE_DB_PATH` | Override database location | Platform default |
...
| `TINYCLAUDE_DEBUG` | Enable debug mode (1/0) - enables console output | 0 |
...
**Solutions**:
1. Stop all tinyclaude processes
