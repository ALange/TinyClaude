...
1. **Global Agents**: Loaded from `~/.config/tinyclaude/agents/` directory
...
The agent system allows tinyclaude to automatically detect and apply model preferences based on the agent being used:

```mermaid
graph TB
    subgraph "Agent Discovery"
        GLOBAL[Global Agents<br/>~/.config/tinyclaude/agents/]
...
tinyclaude implements sophisticated streaming support for handling large language model responses:
...
graph TB
    subgraph "Local Machine"
        SERVER[tinyclaude Server<br/>Port 8080]
        DB[(SQLite DB)]
        LOGS[Log Files]
        CONFIG[Config Files]
...
graph TB
    ...
    subgraph "Application Tier"
        APP1[tinyclaude Instance 1]
        APP2[tinyclaude Instance 2]
        APP3[tinyclaude Instance N]
    end
