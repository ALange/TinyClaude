...
This document outlines the security considerations, practices, and recommendations for the tinyclaude load balancer system.

## ⚠️ Critical Security Notice

**IMPORTANT**: tinyclaude is designed for local development and trusted environments. The current implementation has several security limitations:
...
tinyclaude is a load balancer proxy that manages multiple OAuth accounts to distribute requests to the Claude API. The system handles sensitive authentication tokens and request/response data, requiring careful security considerations.
...
#### 2. Key Management
- Use environment variable for encryption key: `TINYCLAUDE_ENCRYPTION_KEY`
...
**✅ Implemented**: Server now supports the `TINYCLAUDE_HOST` environment variable for binding configuration:
```typescript
// Implemented in apps/server/src/server.ts:480
const hostname = process.env.TINYCLAUDE_HOST || "0.0.0.0"; // Allow binding configuration
const serverConfig = {
    port: runtime.port,
    hostname,
...
export TINYCLAUDE_HOST=127.0.0.1
bun start

# Bind to all interfaces (default - insecure)
export TINYCLAUDE_HOST=0.0.0.0
bun start

# Bind to specific network interface
export TINYCLAUDE_HOST=192.168.1.100
bun start
```
...
server {
    ...
    server_name tinyclaude.internal;
...
- [ ] **Network Configuration**
  - [ ] ✅ Bind server to localhost only using `TINYCLAUDE_HOST=127.0.0.1`
...
**Risk**: Running tinyclaude with default settings exposes it to the network
...
TINYCLAUDE_DEBUG=0            # Set to 1 only for debugging
...
TINYCLAUDE_HOST=0.0.0.0   # Server binding host (use 127.0.0.1 for localhost-only)
...
sqlite3 tinyclaude.db "SELECT COUNT(*) as count, account_used 
FROM requests 
...
sqlite3 tinyclaude.db "SELECT * FROM audit_log WHERE action LIKE '%config%'"
```
...
### Key Takeaways
1. **tinyclaude prioritizes functionality over security** - suitable for development, not production
