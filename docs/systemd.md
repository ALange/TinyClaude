...
Production deployment of tinyclaude as a systemd service on Linux.

## Reference Unit File

```ini
[Unit]
Description=tinyclaude proxy
...
ExecStartPre=/opt/tinyclaude/scripts/preflight-env.sh

# --- Main process ---
# --smol enables aggressive GC (the correct way to reduce memory usage)
ExecStart=/usr/bin/tinyclaude --smol --serve --port 8889
...
ProtectHome=true
ReadWritePaths=/var/lib/tinyclaude
PrivateTmp=true

# --- Logging ---
StandardOutput=journal
StandardError=journal
SyslogIdentifier=tinyclaude
...
When running tinyclaude outside systemd, source the script before starting:

```sh
. /opt/tinyclaude/scripts/preflight-env.sh
exec bun run tinyclaude --smol --serve
```
...
Set `MemoryMax` above what the process actually needs. `MemoryHigh` applies back-pressure before the hard limit. Combined with `--smol`, this keeps tinyclaude within predictable bounds.
...
- After a `StartLimitBurst` failure, manual intervention is required: `systemctl reset-failed tinyclaude && systemctl start tinyclaude`
...
journalctl -u tinyclaude --no-pager -n 20

# Reset the failed state
systemctl reset-failed tinyclaude

# Fix the unit file, then reload and start
systemctl daemon-reload
systemctl start tinyclaude
```
...
The reference unit file uses a dedicated `tinyclaude` user. Create it:

```sh
useradd --system --no-create-home --shell /usr/sbin/nologin tinyclaude
mkdir -p /var/lib/tinyclaude
chown tinyclaude:tinyclaude /var/lib/tinyclaude
```
...
```ini
Environment=TINYCLAUDE_DB_PATH=/var/lib/tinyclaude/tinyclaude.db
```
