...
This guide covers the auto-refresh feature in tinyclaude, which automatically sends dummy messages to Anthropic accounts when their usage windows reset to start a new window.
...
# Monitor logs
tail -f ~/.local/share/tinyclaude/logs/tinyclaude.log | grep "Auto-refresh"
```
...
tail -f ~/.local/share/tinyclaude/logs/tinyclaude.log | grep -E "(Auto-refresh|AutoRefreshScheduler)"
```
...
#!/bin/bash
while true; do
  if tail -n 10 ~/.local/share/tinyclaude/logs/tinyclaude.log | grep -q "Auto-refresh.*failed"; then
