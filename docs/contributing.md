# Contributing to tinyclaude

Welcome to tinyclaude! We're thrilled that you're interested in contributing to our Claude load balancer project. This document provides guidelines and instructions for contributing to the project.
...
First off, thank you for considering contributing to tinyclaude! We welcome contributions from everyone, regardless of their background or experience level.
...
2. **Clone your fork**:
   ```bash
   git clone https://github.com/YOUR_USERNAME/tinyclaude.git
   cd tinyclaude
   ```

3. **Add the upstream remote**:
   ```bash
   git remote add upstream https://github.com/ORIGINAL_OWNER/tinyclaude.git
   ```
...
tinyclaude is organized as a Bun monorepo with clear separation of concerns:
...
- Packages: Prefixed with `@tinyclaude/` (e.g., `@tinyclaude/core`, `@tinyclaude/database`)
...
2. **Path Aliases**:
   ...
   ```typescript
   // Good
   import { Database } from '@tinyclaude/database';
   import { LoadBalancer } from '@tinyclaude/load-balancer';
   import { formatDate } from './utils';
   import type { Account } from '@tinyclaude/types';
...
```bash
# If tinyclaude is not installed globally, use:
# bun run cli [options]
# or build and run with: bun run tinyclaude
...
3. **Import errors**: Ensure you're using workspace imports (`@tinyclaude/*`) for cross-package dependencies
4. **Database issues**: The SQLite database is created automatically in the data directory

Thank you for contributing to tinyclaude! Your efforts help make Claude AI more accessible to everyone.