...
This guide helps you diagnose and resolve common issues with tinyclaude.
...
2. Refresh the token manually:
   - Remove and re-add the account:
     ```bash
     tinyclaude --remove <account-name>
     tinyclaude --add-account <account-name> --mode <mode> --priority <number>
     ```
...
**Solutions**:
1. tinyclaude automatically attempts to refresh expired tokens
...
tinyclaude detects rate limits through response headers and HTTP status codes:
...
cat /tmp/tinyclaude-logs/app.log | grep "rate limited"
...
1. tinyclaude automatically rotates to the next available account
...
1. Add more accounts to your pool:
   ```bash
   tinyclaude --add-account account2 --mode claude-oauth --priority 10
   ```
...
4. Pause/resume accounts as needed:
   ```bash
   tinyclaude --pause <account-name>
   tinyclaude --resume <account-name>
   ```
...
1. Check log file size (auto-rotates at 10MB):
   ```bash
   ls -lh /tmp/tinyclaude-logs/app.log
   ```
...
- **macOS**: `~/.config/tinyclaude/tinyclaude.json`
- **Linux**: `~/.config/tinyclaude/tinyclaude.json`
...
1. Validate JSON syntax:
   ```bash
   cat ~/.config/tinyclaude/tinyclaude.json | jq .
   ```

2. Reset to defaults:
   ```bash
   # Backup current config
   cp ~/.config/tinyclaude/tinyclaude.json ~/.config/tinyclaude/config.backup.json
   # Remove corrupted config
   rm ~/.config/tinyclaude/tinyclaude.json
...
2. If repair succeeds but issues persist, backup and reset:
   ...
   cp ~/.config/tinyclaude/tinyclaude.db ~/.config/tinyclaude/tinyclaude.db.backup
...
3. For severe corruption, recreate the database:
   ```bash
   # Backup first!
   mv ~/.config/tinyclaude/tinyclaude.db ~/.config/tinyclaude/tinyclaude.db.old
...
1. Check database file permissions:
   ```bash
   # macOS/Linux
   ls -la ~/.config/tinyclaude/tinyclaude.db

   # Windows
   dir %LOCALAPPDATA%\tinyclaude\tinyclaude.db
   ```
...
2. Create the directory if it doesn't exist:
   ...
   # Windows
   mkdir %LOCALAPPDATA%\tinyclaude
   ```

3. Use a custom database path:
   ```bash
   export TINYCLAUDE_DB_PATH=/path/to/custom/tinyclaude.db
   bun start
   ```
...
2. If migrations fail repeatedly:
   ...
   rm ~/.config/tinyclaude/tinyclaude.db
   bun start
   ```

3. Check for database corruption:
   ```bash
   sqlite3 ~/.config/tinyclaude/tinyclaude.db "PRAGMA integrity_check;"
   ```
...
3. Check logs for async writer errors:
   ```bash
   grep "async-db-writer" /tmp/tinyclaude-logs/app.log
   ```
...
**Solutions**:
1. Ensure only one instance of tinyclaude is running:
   ```bash
   ps aux | grep "bun start" | grep -v grep
   ps aux | grep "tinyclaude --serve" | grep -v grep
   ```

2. Kill any zombie processes:
   ```bash
   pkill -f "bun start"
   pkill -f "tinyclaude --serve"
   ```

3. Check for hanging database connections:
   ```bash
   lsof ~/.config/tinyclaude/tinyclaude.db
   ```
...
3. Check if streaming is working:
   ...
   grep "Streaming response" /tmp/tinyclaude-logs/app.log
   ```
...
1. Check if requests are being recorded:
   ...
   sqlite3 ~/.config/tinyclaude/tinyclaude.db "SELECT COUNT(*) FROM requests WHERE timestamp > strftime('%s', 'now', '-1 hour') * 1000;"
   ```
...
2. Check for usage extraction errors:
   ```bash
   grep "extractUsageInfo" /tmp/tinyclaude-logs/app.log
   ```

3. Verify model pricing data:
   ```bash
   # Pricing updates every 24 hours by default
   grep "Fetching latest pricing" /tmp/tinyclaude-logs/app.log
   ```
...
- **All platforms**: `/tmp/tinyclaude-logs/app.log`
- **Windows**: `%TEMP%\tinyclaude-logs\app.log`
...
```bash
# View real-time logs
tail -f /tmp/tinyclaude-logs/app.log
```
...
```bash
# View only errors
grep "ERROR" /tmp/tinyclaude-logs/app.log

# View warnings and errors
grep -E "WARN|ERROR" /tmp/tinyclaude-logs/app.log
```

**Filter by component**:
```bash
# View only proxy logs
grep "\[Proxy\]" /tmp/tinyclaude-logs/app.log

# View only server logs
grep "\[Server\]" /tmp/tinyclaude-logs/app.log
```
...
- Check account status: `tinyclaude --list`
...
**Solutions**:
1. Ensure only one tinyclaude instance is running
2. Kill any zombie processes
...
3. Clear history if data is corrupted: `tinyclaude --clear-history`
...
**Solutions**:
1. Validate JSON syntax: `cat ~/.config/tinyclaude/tinyclaude.json | jq .`
...
3. Use custom path: `export TINYCLAUDE_DB_PATH=/custom/path/db.db`
...
| Variable | Description | Default | Example |
|----------|-------------|---------|---------|
| `TINYCLAUDE_CONFIG_PATH` | Custom config file location | Platform-specific | `/opt/tinyclaude/config.json` |
| `TINYCLAUDE_DB_PATH` | Custom database location | Platform-specific | `/opt/tinyclaude/data.db` |

### Logging and Debugging

| Variable | Description | Default | Example |
|----------|-------------|---------|---------|
| `TINYCLAUDE_DEBUG` | Enable debug mode | `0` | `1` |
...
export TINYCLAUDE_CONFIG_PATH=/etc/tinyclaude/config.json
export TINYCLAUDE_DB_PATH=/var/lib/tinyclaude/data.db
export PORT=3000
bun start
...
### Q: How do I know if tinyclaude is working?
...
### Q: Can I use tinyclaude with multiple client applications?

**A**: Yes, tinyclaude acts as a transparent proxy. Point any Claude API client to `http://localhost:8080` instead of `https://api.anthropic.com`.

### Q: How do I backup my accounts?

**A**: The account data is stored in the SQLite database. Backup locations:
- **macOS/Linux**: `~/.config/tinyclaude/tinyclaude.db`
- **Windows**: `%LOCALAPPDATA%\tinyclaude\tinyclaude.db` or `%APPDATA%\tinyclaude\tinyclaude.db`
...
1. Database file (`tinyclaude.db`)
2. Config file (`tinyclaude.json`)
...
4. Check if requests are being recorded: `sqlite3 ~/.config/tinyclaude/tinyclaude.db "SELECT COUNT(*) FROM requests;"`
...
### Q: Can I use tinyclaude in production?
...
5. Check status: `tinyclaude --list`
...
### Q: What's the difference between running with bun start vs tinyclaude --serve?

**A**: Both commands start the server:
- `bun start`: Runs the server using the npm script
- `tinyclaude --serve`: Runs the server directly via the CLI binary
...
2. **Error Logs**:
   ...
   tail -n 100 /tmp/tinyclaude-logs/app.log
   ```

3. **Configuration** (sanitized):
   ```bash
   # Remove sensitive data before sharing
   cat ~/.config/tinyclaude/tinyclaude.json | jq 'del(.client_id)'
   ```
...
env | grep -E "tinyclaude|CLIENT_ID|PORT|LB_STRATEGY|LOG_|PROXY" | sort
echo ""

echo "=== Process Info ==="
ps aux | grep -E "bun start|tinyclaude" | grep -v grep
echo ""
...
echo "=== Database Info ==="
if [ -f "$HOME/.config/tinyclaude/tinyclaude.db" ]; then
    echo "Database size: $(du -h "$HOME/.config/tinyclaude/tinyclaude.db" | cut -f1)"
    echo "Request count: $(sqlite3 "$HOME/.config/tinyclaude/tinyclaude.db" "SELECT COUNT(*) FROM requests;" 2>/dev/null || echo "Could not query")"
    echo "Account count: $(sqlite3 "$HOME/.config/tinyclaude/tinyclaude.db" "SELECT COUNT(*) FROM accounts;" 2>/dev/null || echo "Could not query")"
else
...
if [ -f "/tmp/tinyclaude-logs/app.log" ]; then
    grep "ERROR" /tmp/tinyclaude-logs/app.log | tail -20
else
...
if [ -f "/tmp/tinyclaude-logs/app.log" ]; then
    grep -E "rate.?limit" /tmp/tinyclaude-logs/app.log | tail -10
else
    echo "Log file not found"
fi
echo ""
...
echo "=== Account Status ==="
tinyclaude --list 2>/dev/null || echo "Could not get account list"
echo ""
...
tail -f /tmp/tinyclaude-logs/app.log | grep -E "INFO|WARN|ERROR"
```
...
4. **Rate Limits**
   ```bash
   grep "rate.?limit" /tmp/tinyclaude-logs/app.log | tail -10
   ```
...
| Problem | Quick Fix |
|---------|-----------|
| All accounts rate limited | Add more accounts: `tinyclaude --add-account newaccount --mode claude-oauth --priority 10` |
| Token expired | Re-authenticate: `tinyclaude --remove account && tinyclaude --add-account account --mode claude-oauth --priority 0` |
| Database locked | Kill duplicate processes: `pkill -f "bun start"` |
| Port in use | Use different port: `PORT=3000 bun start` |
| Config corrupted | Reset config: `rm ~/.config/tinyclaude/tinyclaude.json` |
| Analytics missing | Clear history: `tinyclaude --clear-history` |
| Slow responses | Check session duration settings (default 1 hour) |

Remember: Most issues can be resolved by checking logs, verifying account status, and ensuring proper network connectivity. When in doubt, restart the service with debug logging enabled: `TINYCLAUDE_DEBUG=1 LOG_LEVEL=DEBUG bun start`