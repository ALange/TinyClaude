# tinyclaude Deployment Documentation

## Overview

tinyclaude is a load balancer proxy for Claude API accounts that can be deployed in various configurations, from simple local development to production-grade distributed systems. This document covers all deployment options, from single-instance setups to scalable architectures.

> **Important**: tinyclaude provides a powerful CLI interface and web dashboard for monitoring and management. The main executable `tinyclaude` provides all functionality through command-line flags.
...
git clone https://github.com/ALange/TinyClaude.git
cd tinyclaude
...
Compile tinyclaude into a single executable for easy deployment:

```bash
# Build all components (dashboard and CLI)
bun run build

# Build the main tinyclaude binary (includes CLI and server)
cd apps/cli
bun build src/main.ts --compile --outfile dist/tinyclaude --target=bun

# Copy binary to deployment location
cp apps/cli/dist/tinyclaude /opt/tinyclaude/
# Make it executable
chmod +x /opt/tinyclaude/tinyclaude
```
...
```
/opt/tinyclaude/
├── tinyclaude             # Main binary (CLI + Server)
├── config/
│   └── config.json     # Configuration (optional)
└── data/
    ├── tinyclaude.db      # SQLite database
    └── logs/           # Log files (if configured)
```

**Note**: The configuration and database are automatically created in platform-specific directories:
- **Linux/macOS**: `~/.config/tinyclaude/`
- **Windows**: `%LOCALAPPDATA%\tinyclaude\` or `%APPDATA%\tinyclaude\`
...
module.exports = {
  apps: [{
    name: 'tinyclaude',
    script: '/opt/tinyclaude/tinyclaude',
    args: '--serve',
    instances: 1,
    exec_mode: 'fork',
    env: {
    ...
    error_file: '/opt/tinyclaude/data/logs/error.log',
    out_file: '/opt/tinyclaude/data/logs/out.log',
    log_file: '/opt/tinyclaude/data/logs/combined.log',
    time: true,
    autorestart: true,
    max_restarts: 10,
    min_uptime: '10s',
...
```bash
# Create service file
sudo cat > /etc/systemd/system/tinyclaude.service << 'EOF'
[Unit]
Description=tinyclaude Load Balancer
...
User=tinyclaude
Group=tinyclaude
WorkingDirectory=/opt/tinyclaude
ExecStart=/opt/tinyclaude/tinyclaude --serve
Restart=always
RestartSec=5
...
ProtectHome=true
ReadWritePaths=/opt/tinyclaude/data
...
sudo useradd -r -s /bin/false tinyclaude
sudo mkdir -p /opt/tinyclaude/{config,data/logs}
sudo chown -R tinyclaude:tinyclaude /opt/tinyclaude

# Enable and start service
sudo systemctl daemon-reload
sudo systemctl enable tinyclaude
sudo systemctl start tinyclaude
```
...
RUN bun run build
RUN cd apps/server && bun build src/server.ts --compile --outfile dist/tinyclaude-server
RUN cd apps/cli && bun build src/cli.ts --compile --outfile dist/tinyclaude-cli
...
# Create user
RUN useradd -r -s /bin/false tinyclaude

# Copy binary and dashboard
COPY --from=builder /app/apps/cli/dist/tinyclaude /usr/local/bin/tinyclaude
COPY --from=builder /app/packages/dashboard-web/dist /opt/tinyclaude/dashboard

# Set permissions
RUN chmod +x /usr/local/bin/tinyclaude

# Create data directories
RUN mkdir -p /data /config && chown -R tinyclaude:tinyclaude /data /config

USER tinyclaude

# Environment
ENV PORT=8080
ENV TINYCLAUDE_CONFIG_PATH=/config/tinyclaude.json
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD ["/usr/local/bin/tinyclaude-server", "health"] || exit 1

ENTRYPOINT ["/usr/local/bin/tinyclaude", "--serve"]
```
...
services:
  ...
  nginx:
    image: nginx:alpine
    container_name: tinyclaude-nginx
    restart: unless-stopped
    ports:
      - "80:80"
      - "443:443"
...
docker build -t tinyclaude:latest .
...
```nginx
# /etc/nginx/sites-available/tinyclaude
upstream tinyclaude_backend {
...
server {
    listen 80;
    server_name tinyclaude.yourdomain.com;
...
server {
    ...
    # SSL configuration
    ssl_certificate /etc/letsencrypt/live/tinyclaude.yourdomain.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/tinyclaude.yourdomain.com/privkey.pem;
    ...
    # Main proxy
    location / {
        proxy_pass http://tinyclaude_backend;
    }
...
graph TB
    subgraph "Logging Architecture"
        APP[tinyclaude Server]
        
        subgraph "Log Outputs"
            FILE[File Logs<br/>/var/log/tinyclaude/]
...
export const metrics = {
  requestsTotal: new Counter({
    name: 'tinyclaude_requests_total',
    help: 'Total number of requests',
    labelNames: ['method', 'status', 'account']
  }),
  
  requestDuration: new Histogram({
    name: 'tinyclaude_request_duration_seconds',
  ...
  activeAccounts: new Gauge({
    name: 'tinyclaude_active_accounts',
    help: 'Number of active accounts',
    labelNames: ['priority']
  }),
  
  rateLimitedAccounts: new Gauge({
    name: 'tinyclaude_rate_limited_accounts',
...
services:
  ...
  promtail:
    ...
    volumes:
      ...
      - /var/log:/var/log:ro
      - /opt/tinyclaude/data/logs:/app/logs:ro
...
echo "tinyclaude soft nofile 65536" >> /etc/security/limits.conf
echo "tinyclaude hard nofile 65536" >> /etc/security/limits.conf
...
```json
// ~/.config/tinyclaude/config.json (or platform-specific location)
{
  "lb_strategy": "session",
  "client_id": "9d1c250a-e61b-44d9-88ed-5944d1962f5e",
  "retry_attempts": 3,
...
graph TB
    ...
    subgraph "Application Instances"
        APP1[tinyclaude-1<br/>Port 8081]
        APP2[tinyclaude-2<br/>Port 8082]
        APP3[tinyclaude-N<br/>Port 808N]
    end
...
tinyclaude uses SQLite by default, which is suitable for single-instance deployments. For Kubernetes multi-pod deployments, set `DATABASE_URL` to use PostgreSQL (see [PostgreSQL Support for Multi-Pod Deployments](#postgresql-support-for-multi-pod-deployments) above).

#### SQLite Optimization (Default)

```sql
-- These optimizations are automatically applied by tinyclaude
PRAGMA journal_mode = WAL;
PRAGMA synchronous = NORMAL;
...
SQLite is unsuitable for Kubernetes deployments with multiple replicas because pods cannot safely share a single file. tinyclaude has first-class PostgreSQL support via `Bun.SQL` — set the `DATABASE_URL` environment variable and the schema is created automatically on startup.

```bash
# Create a PostgreSQL database
psql -U postgres -c "CREATE DATABASE tinyclaude;"
psql -U postgres -c "CREATE USER tinyclaude_user WITH PASSWORD 'secret';"
psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE tinyclaude TO tinyclaude_user;"

# Set DATABASE_URL — that's all that's needed
export DATABASE_URL=postgresql://tinyclaude_user:secret@postgres-host:5432/tinyclaude
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: tinyclaude
        image: your-registry/tinyclaude:latest
        ports:
        - containerPort: 8080
        env:
        - name: DATABASE_URL
...
type: Opaque
stringData:
  database-url: "postgresql://tinyclaude_user:secret@postgres-svc:5432/tinyclaude"
```
...
tinyclaude provides a health check endpoint for monitoring:
...
1. **Database Lock Errors**
   ...
   # Linux/macOS:
   sqlite3 ~/.config/tinyclaude/tinyclaude.db "PRAGMA journal_mode=WAL;"
   
   # Windows:
   sqlite3 %LOCALAPPDATA%\tinyclaude\tinyclaude.db "PRAGMA journal_mode=WAL;"
   ```

2. **High Memory Usage**
   ```bash
   # Check for memory leaks
   node --inspect=0.0.0.0:9229 /opt/tinyclaude/tinyclaude-server
   ```

3. **Connection Refused**
   ```bash
   # Check if service is running
   systemctl status tinyclaude
   # Check logs
   journalctl -u tinyclaude -f
   ```
...
tinyclaude --logs 100 | grep ERROR

# Weekly: Database maintenance
# Linux/macOS:
sqlite3 ~/.config/tinyclaude/tinyclaude.db "VACUUM;"
sqlite3 ~/.config/tinyclaude/tinyclaude.db "ANALYZE;"

# Monthly: Clean old request history
tinyclaude --clear-history

# Quarterly: Update dependencies (if running from source)
cd /path/to/tinyclaude
bun update
```
...
BACKUP_DIR="/backup/tinyclaude/$(date +%Y%m%d)"
mkdir -p "$BACKUP_DIR"

# Determine config directory based on OS
if [[ "$OSTYPE" == "darwin"* ]] || [[ "$OSTYPE" == "linux-gnu"* ]]; then
    CONFIG_DIR="$HOME/.config/tinyclaude"
elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
    CONFIG_DIR="$LOCALAPPDATA/tinyclaude"
else
    CONFIG_DIR="$HOME/.config/tinyclaude"  # Default fallback
fi

# Backup database and config
sqlite3 "$CONFIG_DIR/tinyclaude.db" ".backup $BACKUP_DIR/tinyclaude.db"
...
find /backup/tinyclaude -name "*.tar.gz" -mtime +30 -delete
```
...
| `TINYCLAUDE_DEBUG` | 0 | Enable debug mode (1/0) - enables console output |
...
| `TINYCLAUDE_CONFIG_PATH` | Platform-specific | Path to configuration file |
| `TINYCLAUDE_DB_PATH` | Platform-specific | Path to SQLite database file (ignored when `DATABASE_URL` is set) |
...
tinyclaude also supports a JSON configuration file that takes precedence over environment variables:
...
- **Linux/macOS**: `~/.config/tinyclaude/config.json`
- **Windows**: `%APPDATA%\tinyclaude\config.json`

## Conclusion

tinyclaude is designed to be flexible and scalable, supporting everything from simple local deployments to complex distributed architectures. Choose the deployment option that best fits your needs and scale as your requirements grow.
...
- [GitHub Repository](https://github.com/ALange/TinyClaude)

## Web Dashboard

tinyclaude includes a powerful web-based dashboard for monitoring and management.

### Accessing the Dashboard

```bash
# Start tinyclaude server with embedded dashboard
tinyclaude --serve
...
The tinyclaude CLI can connect to a remote API server for distributed deployments:

```bash
# Set API URL for remote connection
export TINYCLAUDE_API_URL=https://tinyclaude.example.com
tinyclaude --list  # Will query the remote server
```
