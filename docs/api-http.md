# tinyclaude HTTP API Documentation
...
tinyclaude provides a RESTful HTTP API for managing accounts, monitoring usage, and proxying requests to Claude. The API runs on port 8080 by default and requires no authentication.
...
Check the health status of the tinyclaude service.
...
- `previous_response_id` is accepted but ignored — tinyclaude is stateless and does not store prior responses. Codex uses this field only over WebSocket (which tinyclaude does not implement); for regular HTTP requests Codex always sends the full conversation history in `input`, so this field has no effect.
...
**Note:** `GET /v1/models` is proxied through to Claude like any other `/v1/*` request. A successful response (from a console/API-key account) is also passively captured into tinyclaude's own model catalog cache — see [Model Catalog](#model-catalog) below for the read endpoint (`GET /api/models`) and how the catalog is kept up to date.

**Headers:**
- All standard Claude API headers are supported
- `Authorization` header is managed by tinyclaude (no need to provide)
...
**Automatic Failover:**
If a request fails or an account is rate limited, tinyclaude automatically retries with the next available account according to the configured load balancing strategy. This ensures high availability and reliability.
...
When an account hits rate limits, tinyclaude automatically fails over to the next available account. If all accounts are rate limited, a 503 error is returned.
...
tinyclaude automatically captures streaming response bodies for analytics and debugging purposes:
...
tinyclaude can be configured using the following environment variables:
...
- `TINYCLAUDE_MODELS_REFRESH_HOURS` - Hours between scheduled model catalog refreshes, 0 disables scheduled refresh (default: 168 / 7 days). See [Model Catalog](configuration.md#model-catalog).
- `TINYCLAUDE_MODELS_OFFLINE` - Disable scheduled/manual model catalog refresh and passive `/v1/models` capture (default: unset)
- `TINYCLAUDE_MODELS_CACHE_DIR` - Directory for the persisted model catalog cache file (default: platform config dir)
- `TINYCLAUDE_MODELS_OAUTH_REFRESH` - Allow OAuth accounts as a fallback source for manual model catalog refreshes (default: unset / console-only)

### Configuration File

In addition to environment variables, tinyclaude supports configuration through a JSON file. The config file location varies by platform:
- macOS: `~/Library/Application Support/tinyclaude/config.json`
- Linux: `~/.config/tinyclaude/config.json`
...
1. **No Authentication**: The API endpoints do not require authentication. tinyclaude manages the OAuth tokens internally for proxying to Claude.

2. **Automatic Failover**: When a request fails or an account is rate limited, tinyclaude automatically tries the next available account. If no accounts are available, requests are forwarded without authentication as a fallback.
