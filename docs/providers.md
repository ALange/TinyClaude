...
The tinyclaude providers system is a modular architecture designed to support multiple AI service providers through a unified interface. Currently, it implements support for Anthropic's services through a single provider that can operate in two modes:
...
### Architecture
```
Anthropic Client → tinyclaude Proxy → OpenAI Provider (OpenRouter, etc.) → Transformed Response → Client
```
...
**OpenRouter:**
```bash
tinyclaude --add-account openrouter-account \
...
**Together AI:**
```bash
tinyclaude --add-account together-account \
  --provider openai-compatible \
  --api-key ... \
...
```bash
tinyclaude --add-account local-models \
  --provider openai-compatible \
  --api-key dummy-key \
...
tinyclaude --add-account custom-provider \
  --provider openai-compatible \
  --api-key sk-... \
...
tinyclaude uses an account priority system to control request routing order:
...
tinyclaude --add-account secondary-account --mode max --priority 10
tinyclaude --add-account backup-account --mode max --priority 50
```
