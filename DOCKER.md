...
This guide covers how to use tinyclaude with Docker for easy deployment across multiple architectures.
...
- `TINYCLAUDE_DB_PATH` - Database file path (default: `/data/tinyclaude.db`)
...
tinyclaude Docker containers now support `.env` files for easy configuration management:
...
curl -O https://raw.githubusercontent.com/ALange/TinyClaude/main/.env.example
mv .env.example .env
...
```bash
docker run -v /path/on/host:/data ghcr.io/alange/tinyclaude:latest
```
...
```bash
# List accounts
docker exec -it tinyclaude tinyclaude --list

# Remove an account
docker exec -it tinyclaude tinyclaude --remove myaccount

# Set priority
docker exec -it tinyclaude tinyclaude --set-priority myaccount 5
```
...
```bash
docker run -v ~/.config/tinyclaude:/data ghcr.io/alange/tinyclaude:latest
```
...
docker run -p 8080:8080 tinyclaude:local
```
...
docker buildx build \
  ...
  --tag ghcr.io/alange/tinyclaude:latest \
  --push \
  .
```
...
docker inspect --format='{{json .State.Health}}' tinyclaude
```
...
```bash
docker exec tinyclaude ls -la /data
```
...
```bash
docker run -p 8081:8080 ghcr.io/alange/tinyclaude:latest
```
...
# Backup database
docker cp tinyclaude:/data/tinyclaude.db ./backup-$(date +%Y%m%d).db
```
